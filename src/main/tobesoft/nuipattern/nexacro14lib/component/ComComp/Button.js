...
//==============================================================================
// nexacro.Button && nexacro.Button_Style
//==============================================================================
if (!nexacro.Button) 
{
	//==============================================================================
	// nexacro.Button_Style
	//==============================================================================
	...
	//===============================================================
	// nexacro.Button : Button's Create & Update
	//---------------------------------------------------------------
	...
	if (nexacro.Browser == "IE" && nexacro.BrowserVersion >= 10)    // 시점 차이로 인해 미리 로드함.
	{
	    _pButton._preloadImage = function ()
	    {
	        ...
	        for (var i = 0; i < pseudoarr_len; i++)
	        {
	            ...
	            if (val)
	            {
	                ...
	                // load image
	                nexacro._getImageSize(val, function () { }, this);
	            }
	        }
	    };
	}
	...
	_pButton.destroyComponent = function ()
	{
	    ...
	    // TODO check parent에서 제거되기 전에 작업해야 한다.
	    var mainform = this._getMainForm();
	    if (mainform)
	    {
	...
	//===============================================================
	// nexacro.Button : Button's Custom Part
    //---------------------------------------------------------------

	_pButton._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey) // 2013-04-23 pss - override for keydown 'enter'
	{
	    return { want_tab: false, want_return: true, want_escape: false, want_chars: false, want_arrows: false };
	};

    // 2013-04-15 pss - make for groupbox
	...
	_pButton._load_image = function (image) 
	{
		...
		if (control_elem && image) 
		{
			...
			if (val) 
			{
			    ...
			    // load image
				this._lastest_imgurl = val; // [REQ_36181] 2014-05-28 async loading order revise
	...
	// for pre loaded image ==> directly called from on_apply_style_image()
	// for non pre loaded image : img load is async ==> this function called from event handler
	...
	//===============================================================
	// nexacro.Button : Button's Properties
	//---------------------------------------------------------------
	_pButton.on_apply_text = function () 
	{
	    ...
	    if (control_elem)
	    {
			...
			if (!text_elem)
			{
				...
				this._text_elem = text_elem;
				this.on_apply_wordwrap();       //REQ-32570 colorart 2013-06-05
    ...
    // Hotkey가 눌렸을때 DefaultAction 수행 2013.06.25 neoarc
	...
	//---------------------------------------------------------------
	// nexacro.Button : Button's Custom Properties
	...
	//===============================================================
	// nexacro.Button : Button's Methods
    //---------------------------------------------------------------
    ...
    //===============================================================
    // nexacro.Button : Button's Events
    //---------------------------------------------------------------
	_pButton.on_fire_sys_onkeyup = function (key_code, alt_key, ctrl_key, shift_key, from_comp, from_refer_comp) //  override for keyup 'enter' or 'space'
	{
	    ...
	    if (!this._is_subcontrol && !this._is_hotkey_click) // subcontrol don't click action (spec) but fileupload is exception
	    {
	...
	{	 
	    var preSelect = this.selectStatus;  // 이전 select값을 리턴해야함 
	    
		if(select)	this._stat_change("select", "selected");
		else this._stat_change("notselect", "normal");
	    // 이전 select값을 리턴해야함 
	    //-----------------------------------------------------------
	...
	//===============================================================
	// nexacro.ImageButton : Style Part
	//---------------------------------------------------------------
	...
	{
	    if (this._img_elem && !(this.getElement()._vml_elem)) //IE8 tileBar min, max button img pos
	    {
	...
	//===============================================================
	// nexacro.ImageButton : Button's Custom Part
	//---------------------------------------------------------------
	...
	//===============================================================
	// nexacro.ImageButton : Button's Properties
	//---------------------------------------------------------------
    ...
    {
        if (enable == undefined) enable = this.enable; 
	...
	{
        if(this.parent._set_real) return; 
