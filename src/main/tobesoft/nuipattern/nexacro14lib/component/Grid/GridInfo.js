...
if (!nexacro.GridFormat)
{
    ...
    nexacro.GridCellInfo = function (id, band, grid, type, idx)
    {
        ...
        this._control_element = true;   // style 적용을 위한 임시조치
    ...
    _pGridCellInfo._query_pseudo_background = function (rowidx, odd, selected, pseudo)
    {
        ...
        if (backgroundimage && !backgroundimage._is_empty)
        {
            ...
            //background = nexacro.$getCachedStyleObj("background", val);		 기존 obj를 사용하면 갱신이 안되는 문제가 생긴다 계속 new해줘야함.

            if (this._bkground)
                delete this._bkground;

            //   background = this._bkground = new nexacro.Style_backgroundBase(val);
            ...
            background.position_x = x;	// position은 복사안됨.
    ...
    _pGridCellInfo._query_pseudo_control = function (control, rowidx, controlProp, styleProp, pseudo, returnType, childctrl, odd, selected)
    {
        ...
        if (controlProp == styleProp)   // gridtree
        {
            if (selected)
            {
        ...
        if (this.isStyleEmpty(result))
        {
            ...
            {
                propobj = style.align;  // inline이 있을 때만.. band의 cellalign도 고려해야되면 추가해야됨.
                if (propobj)
                    this._addStyleCache(styleid, propobj);
        ...
        if (this.isStyleEmpty(result))  // ctrl안의 childctrl일 경우 (ex : comboctrl의 comboedit)
        {
            if (childctrl)
            {
    ...
    _pGridCellInfo.isStyleEmpty = function (prop)  // 임시 필요
    {
    ...
    _pGridCellInfo._query_pseudo_border = function (rowidx, selected, pseudo, suppress, parentcolspan, parentrowspan)
    {
        ...
        if (!linetype || linetype._bindtype > 0)
            linetype = this._searchStyleValue("linetype", styleid, rowidx, false, selected, pseudo, true, exprbind, true);     // border는 cache 처리 따로 함.

        var exprbindproc = (exprbind.type == 0) ? false : true;

        if (!line || line._bindtype > 0)
            line = this._searchStyleValue("border", styleid2, rowidx, false, selected, pseudo, true, exprbind, true);   // line -> border
        ...
        {
            if (linetype._value == "exhorz" || linetype._value == "nocross")    // for html5
            {
                linetype_val = "exvert";
            }
        ...
        if (line && !line._is_empty)
        {
            ...
            /*
            switch (line._linecnt)
            {
                case 1: // [top, right, bottom, left]
                    topstr = line._value;
                    rightstr = line._value;
                    bottomstr = line._value;
                    leftstr = line._value;
                    break;
                case 2: // [top, bottom], [right, left]
                    arrstr = line._value.split(",");
                    topstr = arrstr[0];
                    rightstr = arrstr[1];
                    bottomstr = arrstr[0];
                    leftstr = arrstr[1];
    ...
    _pGridCellInfo.set_style = function (v)
    {
        ...
        v = nexacro._decodeXml(v); // [REQ20550]2011.10.18 "&apos;" 
        ...
        //var style = new nexacro.GridCell_Style(this);
        var style = this.style;
    ...
    {
        this._number_type = 0; // 0: integer, 1: decimal
        this._idx_dot = -1;
    ...
    _pGridCellInfo.__get_view_zeromask = function (mask)
    {
        ...
        return { integer: integer, decimal: decimal, nineidx: nineidx };
    };
    ...
    _pGridCellInfo.__make_text_zeromask = function (text, mask)
    {
        ...
        var peridx = tempmask.indexOf("%");     // 앞뒤로만 붙게 지원 2013/10/18 cmc
        var per = "";
        ...
        if (this._org_val != undefined)
        {
            ...
            // zero값 없애기(Grid에서는 zero값은 없앤다. 소숫점 앞자리만..
            // mask가 #이 있을때
            ///////////////////////////////////////////////
	        ...
	        */
            ...
            if (decimal_flag)
            {
                ...
                //view_str += text.substring(idx_dot, i+1);        	
        ...
        if (this._m_idx_dot != -1)
        {
            if (this._idx_dot == -1 && ret.decimal > 0 &&
        			(this._m_idx_dot != undefined && this._m_idx_dot != -1))
                str += ".";
    ...
    _pGridCellInfo.__update_dot_display = function (v, mask)
    {
        ...
        var len = v.length;
        //        if (this._m_idx_dot != -1 && this._idx_dot != -1) {
    ...
    _pGridCellInfo.__applyMask = function (value, mask, maskchar)
    {
        ...
        for (var i = 0; i < maskLen; i++)
        {
            ...
            if (this.__isMask(maskCh) || maskCh == "\\" || maskCh == "\"" || maskCh == "\'")
            {
                ...
                if (n < valueLen)
                {
                    ...
                    {
                        n--; //value가 입력되지 않으면 이전으로 count 초기화
                        text += maskchar;
                    }
                }
                else
        ...
        //text = this.__make_text_zeromask(text, mask);
    ...
    _pGridCellInfo._getDisplayText_number = function (rowidx)
    {
    	...
    	if (!this.grid._editmasknumber)
        {
    	    this.grid._masknumber_api = new nexacro.EditMaskNumber(null);
        }

    	var masknumber_api = this.grid._masknumber_api;
    	masknumber_api._setLocale(locale);
    	masknumber_api.setMask(mask);
    	var val = masknumber_api.normalizeValue(v.toString(), true);
    	var num = masknumber_api.makeText(val, false);
...
//     	{
//     	    var temp = parseFloat(num); // dataset col type이 string일 경우 NaN check를 위해 변환
// 
//     	    if (isNaN(temp))
//     	        return num;
// 
...
//     	return num;
    ...
    _pGridCellInfo._getPureNumberMask = function (mask)
    {
        ...
        return { "pre_str": pre_str, "pure_mask": pure_mask, "post_str": post_str };
    };
        ...
        else
        {
            ...
            if (null_test == 1)
            {
                if (colType == undefined)
                {
                    ...
                    if (strVal.length <= 6) //time
                        colType = 0;
                    else if (strVal.length <= 8) //date
                        colType = 2;
                    else //datetime
                        colType = 1;
                }
    ...
    _pGridCellInfo._getDisplayText_combo = function (rowidx)
    {
        ...
        if (combodataset && combodataset.length && combocodecol && combocodecol.length && combodatacol && combodatacol.length)
        {
            ...
            this._tempinnerds.copyData(ds, false); //선택안된 row의 lookup을 위한 임시 dataset

            if (ds && this._editingRow == rowidx) // 선택된 row에만 lookup을 수행
...
//      this._updateFormatStr(); 스펙변경시 todo
    ...
    _pGridFormat._loadFromDOM = function (formatElem)
    {
        ...
        for (i = 0; i < len; i++)
        {
            ...
            if (attrval) bandobj.set_style(attrval);
            attrval = bandElem.getAttribute("tooltiptext");
            if (attrval) bandobj.set_tooltiptext(attrval);
            attrval = bandElem.getAttribute("tooltiptype");
            if (attrval) bandobj.set_tooltiptype(attrval);
            attrval = bandElem.getAttribute("cellwordwrap");
            if (attrval) bandobj.set_cellwordwrap(attrval);
            ...
            for (j = 0; j < cnt; j++)
            {
                ...
                if (attrval) cellobj.set_displaytype(attrval);
                attrval = cellElem.getAttribute("edittype");
                if (attrval) cellobj.set_edittype(attrval);
                attrval = cellElem.getAttribute("style");
                if (attrval) cellobj.set_style(attrval);
                attrval = cellElem.getAttribute("tooltiptext");
                if (attrval) cellobj.set_tooltiptext(attrval);
                attrval = cellElem.getAttribute("tooltiptype");
                if (attrval) cellobj.set_tooltiptype(attrval);
                attrval = cellElem.getAttribute("displayexpdec");
                if (attrval) cellobj.set_displayexpdec(attrval);
                attrval = cellElem.getAttribute("locale");
                if (attrval) cellobj.set_locale(attrval);
                attrval = cellElem.getAttribute("mask");
                if (attrval) cellobj.set_mask(attrval);
                attrval = cellElem.getAttribute("maskchar");
                if (attrval) cellobj.set_maskchar(attrval);
                attrval = cellElem.getAttribute("suppress");
                if (attrval) cellobj.set_suppress(attrval);
                attrval = cellElem.getAttribute("suppressalign");
                if (attrval) cellobj.set_suppressalign(attrval);
                attrval = cellElem.getAttribute("suppressedit");
                if (attrval) cellobj.set_suppressedit(attrval);
                attrval = cellElem.getAttribute("wordwrap");
                if (attrval) cellobj.set_wordwrap(attrval);
                attrval = cellElem.getAttribute("expr");
                if (attrval) cellobj.set_expr(attrval);
                attrval = cellElem.getAttribute("text");
                if (attrval) cellobj.set_text(attrval);
                attrval = cellElem.getAttribute("subsumtext");
                if (attrval) cellobj.set_subsumtext(attrval);

                attrval = cellElem.getAttribute("calendardisplay");
                if (attrval) cellobj.set_calendardisplay(attrval);
                attrval = cellElem.getAttribute("calendardisplaynulltext");
                if (attrval) cellobj.set_calendardisplaynulltext(attrval);
                attrval = cellElem.getAttribute("calendardisplaynulltype");
                if (attrval) cellobj.set_calendardisplaynulltype(attrval);

                attrval = cellElem.getAttribute("combodataset");
                if (attrval) cellobj.set_combodataset(attrval);
                attrval = cellElem.getAttribute("combocodecol");
                if (attrval) cellobj.set_combocodecol(attrval);
                attrval = cellElem.getAttribute("combodatacol");
                if (attrval) cellobj.set_combodatacol(attrval);
                attrval = cellElem.getAttribute("combodisplay");
                if (attrval) cellobj.set_combodisplay(attrval);
                attrval = cellElem.getAttribute("combodisplaynulltext");
                if (attrval) cellobj.set_combodisplaynulltext(attrval);
                attrval = cellElem.getAttribute("combodisplaynulltype");
                if (attrval) cellobj.set_combodisplaynulltype(attrval);
                attrval = cellElem.getAttribute("combodisplayrowcount");
                if (attrval) cellobj.set_combodisplayrowcount(attrval);
                attrval = cellElem.getAttribute("combotype");
                if (attrval) cellobj.set_combotype(attrval);

                attrval = cellElem.getAttribute("editacceptsenter");
                if (attrval) cellobj.set_editacceptsenter(attrval);
                attrval = cellElem.getAttribute("editacceptstab");
                if (attrval) cellobj.set_editacceptstab(attrval);
                attrval = cellElem.getAttribute("editautoselect");
                if (attrval) cellobj.set_editautoselect(attrval);
                attrval = cellElem.getAttribute("editautoskip");
                if (attrval) cellobj.set_editautoskip(attrval);
                attrval = cellElem.getAttribute("editclipmode");
                if (attrval) cellobj.set_editclipmode(attrval);
                attrval = cellElem.getAttribute("editdisplay");
                if (attrval) cellobj.set_editdisplay(attrval);
                attrval = cellElem.getAttribute("editfilter");
                if (attrval) cellobj.set_editfilter(attrval);
                attrval = cellElem.getAttribute("editimemode");
                if (attrval) cellobj.set_editimemode(attrval);
                attrval = cellElem.getAttribute("editlimit");
                if (attrval) cellobj.set_editlimit(attrval);
                attrval = cellElem.getAttribute("editlengthunit");
                if (attrval) cellobj.set_editlengthunit(attrval);
                attrval = cellElem.getAttribute("editlimitbymask");
                if (attrval) cellobj.set_editlimitbymask(attrval);
                attrval = cellElem.getAttribute("editscrollbar");
                if (attrval) cellobj.set_editscrollbar(attrval);
                attrval = cellElem.getAttribute("edittrimtype");
                if (attrval) cellobj.set_edittrimtype(attrval);
                attrval = cellElem.getAttribute("edituseime");
                if (attrval) cellobj.set_edituseime(attrval);

                attrval = cellElem.getAttribute("treecheck");
                if (attrval) cellobj.set_treecheck(attrval);
                attrval = cellElem.getAttribute("treecollapseimage");
                if (attrval) cellobj.set_treecollapseimage(attrval);
                attrval = cellElem.getAttribute("treeexpandimage");
                if (attrval) cellobj.set_treeexpandimage(attrval);
                attrval = cellElem.getAttribute("treeitemimage");
                if (attrval) cellobj.set_treeitemimage(attrval);
                attrval = cellElem.getAttribute("treeitemmargin");
                if (attrval) cellobj.set_treeitemmargin(attrval);
                attrval = cellElem.getAttribute("treelevel");
                if (attrval) cellobj.set_treelevel(attrval);
                attrval = cellElem.getAttribute("treestartlevel");
                if (attrval) cellobj.set_treestartlevel(attrval);
                attrval = cellElem.getAttribute("treestate");
                if (attrval) cellobj.set_treestate(attrval);

                attrval = cellElem.getAttribute("expandchar");
                if (attrval) cellobj.set_expandchar(attrval);
                attrval = cellElem.getAttribute("expandimage");
                if (attrval) cellobj.set_expandimage(attrval);
                attrval = cellElem.getAttribute("expandshow");
                if (attrval) cellobj.set_expandshow(attrval);
                attrval = cellElem.getAttribute("expandsize");
                if (attrval) cellobj.set_expandsize(attrval);

                attrval = cellElem.getAttribute("autosizecol");
                if (attrval) cellobj.set_autosizecol(attrval);
                attrval = cellElem.getAttribute("autosizerow");
                if (attrval) cellobj.set_autosizerow(attrval);

                attrval = cellElem.getAttribute("celltype");
                if (attrval) cellobj.set_celltype(attrval);

                attrval = cellElem.getAttribute("cssclass");
                if (attrval) cellobj.set_cssclass(attrval);
