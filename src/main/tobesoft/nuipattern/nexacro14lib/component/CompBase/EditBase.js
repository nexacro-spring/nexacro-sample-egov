...
if (!nexacro.EditBase)
{
    ...
    //==============================================================================
    // nexacro.EditBase : Component's basic flags & Utility Functions
    //==============================================================================
    ...
    _pEditBase.setValue = function (v)
    {
        ...
        if (nexacro._isNull(v) || v === "")
        {
            ...
            if (elem && nexacro._checkActiveElement(elem))
    ...
    {
        //this._stat_mouse.setStatus(nexacro.EditBase.Status.MouseMove);
        this._setCaret(elem);
    };
    ...
    if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 10)
    {
        if (nexacro.OSVersion >= 6.0)
        {
            if (nexacro.BrowserVersion == 10)
			{
				...
				{
                    //trace("WIN7 IE10 _on_default_input_keyinput : " + elem.getElementValue() + " : " + this.comp.id);
					...
					if (!editbase_text) editbase_text = "";
					if (!editbase_value) editbase_value = "";
					...
					if (element_value == editbase_text && element_value == editbase_value)
					{
						this._setCaret(elem);
						return false;
					}
			...
			else if (nexacro.BrowserVersion == 9)
            {
                _pEditBase._on_default_input_keyinput = function (elem)
                {
                    //trace("WIN7 IE9 _on_default_input_keyinput : " + elem.getElementValue());
                    if (this._accept_keyinput_event === false)
                    {
                        this._accept_keyinput_event = true;
                        return false;
            ...
            else
            {
                _pEditBase._on_default_input_keyinput = function (elem)
                {
                    //trace("WIN7 IE8 _on_default_input_keyinput : " + elem.getElementValue() + " : " + this._accept_keyinput_event);
                    if (this._accept_keyinput_event === false)
                    {
                        this._accept_keyinput_event = true;
                        return false;
        ...
        else
        {
            _pEditBase._on_default_input_keyinput = function (elem)
            {
                //trace("WINXP IE _on_default_input_keyinput : ");
                if (this._accept_keyinput_event === false)
                {
                    this._accept_keyinput_event = true;
                    return false;
    ...
    else if (nexacro.OS == "iOS")
    {
        if (nexacro.OSVersion.charAt(0) < 7)
        {
            _pEditBase._on_default_input_keyinput = function (elem)
            {
                //trace("IOS6 _on_default_input_keyinput : ");
                if (this._accept_keyinput_event === false)
                {
                    this._accept_keyinput_event = true;
                    return false;
                ...
                if (this._is_composition() && this._imefirsttxt == elem_value)
                {
                    return false;
                }
        ...
        else
        {
            _pEditBase._on_default_input_keyinput = function (elem)
            {
                //trace("IOS7 _on_default_input_keyinput : ");
                if (this._accept_keyinput_event === false)
                {
                    this._accept_keyinput_event = true;
                    return false;
    ...
    else
    {
        _pEditBase._on_default_input_keyinput = function (elem)
        {
            //trace("ETC _on_default_input_keyinput : " + elem.getElementValue());
            if (this._accept_keyinput_event === false)
            {
                this._accept_keyinput_event = true;
                return false;
            ...
            if (!this._is_composition() && element_value == editbase_text && element_value == editbase_value)
            {
            	this._setCaret(elem);
            	this.onUpdateStyle(comp);
                return false;
    ...
    if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11)
    {
        ...
        {
            //trace("_on_default_input_compositionstart")
    ...
    else if (nexacro.OS == "Android")
    {
        _pEditBase._on_default_input_compositionstart = function (data)
        {
            if (this._keycode == 0)
            {
                // keydown 발생 이전에 keyinput 발생한 경우 caret 갱신
    ...
    if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11)
    {
        ...
        {
        	//trace("_on_default_input_compositionupdate : " + this.comp._input_element.getElementValue());
    ...
    if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11)
    {
        ...
        {
            //trace("_on_default_input_compositionend");
            ...
            if (ret.bTextchangeRet)
            {
                ...
                {                    
                    if (ret.bCancahrchangeRet && ret.bOncharRet)
                    {
                        ...
                        if (!this._is_undo)
                        {
                            var undo_pos = begin_pos + text_info.chartext.length;
        ...
        if (nexacro.OSVersion >= 6.0)
        {
            if (nexacro.SystemLang == "ja")
            {
                if (nexacro.BrowserVersion >= 9)
                {
                    ...
                    {
                        return { newText: newText, bInsert: bInsert, bCut: bCut };
                    };
                ...
                else
                {
                    ...
                    _pEditBase._after_filterprocess_from_text_event = function (chartext, newText, bInsert, bCut)
                    {
                        //chartext가 없을 경우 입력 취소.
                        if (!chartext && !this._is_composition() && !this._is_undo)
                        {
                            this._text = this._value;
                            return { newText: this._value, bInsert: false, bCut: false };
                        }
                        return { newText: newText, bInsert: bInsert, bCut: bCut };
                    };
    ...
    if (nexacro.Browser == "Safari" || nexacro.Browser == "MobileSafari" || nexacro.Browser == "IE")
    {
        _pEditBase._apply_autoskip = function ()
        {
            var pThis = this;
            nexacro.OnceCallbackTimer.callonce(pThis.comp, function () { pThis.comp.on_apply_autoskip(); }, 0);
        };
    }
    else
    {
    ...
    {
        if (application._input_paste_comp == this.comp)
    ...
    _pEditNormal.setSelectText = function (elem, v)
    {
        if (v && v.length > 0)
        {
            ...
            if (str.length > 0)
            {
                this._text = curTxt.substring(0, start) + str + curTxt.substr(end);
                ...
                if (!elem.setElementSetSelect(this._begin_pos, this._end_pos))
                    this._accept_focus_event = false;
    ...
    _pEditNormal.getLength = function (elem, str, bStr, checkLen, unitGbn)
    {
        ...
        if (unitGbn === undefined) unitGbn = this._lengthunit;
        if (bStr)
        {
            if (unitGbn == "utf8")
            {
            ...
            else
            {
            	...
            	if (checkLen === undefined) checkLen = comp.maxlength;
                var ret = str;
                if (checkLen < str.length)
                {
                    ret = ret.substring(0, checkLen);
                }
                return { "len": str.length, "str": ret };
            }
        }
        else
        {
    ...
    _pEditNormal.utf8ByteCount = function (elem, str, bRetStr, checkLen)
    {
        ...
        if (bRetStr)
        {
            ...
            return { "len": count, "str": str.substring(0, stopPos + 1) };
        }
    ...
    _pEditNormal.asciiByteCount = function (elem, str, bRetStr, checkLen)
    {
        ...
        if (bRetStr)
        {
            ...
            return { "len": j, "str": str.substring(0, stopPos + 1) };
        }
        else
        {
            for (i = 0; i < str.length; i++)
    ...
    _pEditNormal.deleteChar = function (bBack)
    {
        ...
        if (bBack)
        {
            ...
            {
                begin_pos--;
                if (text.length > 0 && begin_pos > 0)
                {
                    if (text.substring(begin_pos - 1, begin_pos) == '\r')
                    {
                        begin_pos--;
                    }
                }
        ...
        else
        {
            ...
            {
                end_pos++;
                if (text.length > 0 && end_pos < text.length)
                {
                    if (text.substring(end_pos, end_pos + 1) == '\n')
                    {
                        end_pos++;
                    }
                }
    ...
    _pEditNormal._on_default_input_keydown = function (elem, keyCode, altkey, ctrlkey, shiftkey)
    {
        ...
        if (keyCode == nexacro.KeyCode_ImeInput)
        {
            if (nexacro.Browser == "IE")
            {
                ...
                {
                    //XP에서 propertychange이후에 keydown이 발생하기떄문에 caretposition이 다른브라우저와 다름.
        ...
        {
            if (this._is_composition() && (keyCode == 8 || keyCode == 46 || (keyCode >= 33 && keyCode <= 40)))
            {
                this._on_input_compositionend("");
                this._stat_composition.init();
                this._compositionend_value = "";
    ...
    {        
        ...
        else if ((nexacro.OS == "iOS" && !(keyCode == nexacro.Event.KEY_ENTER)) || keyCode == nexacro.Event.KEY_SPACE)
        {
    ...
    _pEditNormal._on_default_input_keyup = function (elem, keycode, altkey, ctrlkey, shiftkey)
    {
        ...
        if (nexacro.Browser == "IE" && nexacro.BrowserVersion >= 9 && (k == 8 || k == 46))
        {
        ...
        if (nexacro.Browser == "IE" && nexacro.OSVersion < 6.0)
        {
            ...
            {
                var pThis = this;
                nexacro.OnceCallbackTimer.callonce(pThis.comp, function () { pThis._setCaret(elem); }, 0);
            }
        }
        else if (nexacro.OS == "iOS")
        {
	...
	{
		if (!insertText || insertText.length == 0) return true;
    ...
    //==============================================================================
    // nexacro.MultiEdit : textarea use
    //==============================================================================
    ...
    _pMultilineEdit._destroy = function ()
    {
        if (application._input_paste_comp == this.comp)
            application._input_paste_comp = null;
    ...
    _pMultilineEdit.setPosition = function ()
    {
        ...
        if (elem && elem._handle)
        {
            ...
            // textarea halign has always default value 'top'.
    ...
    //==============================================================================
    // nexacro.EditMaskString : mask type string use
    //==============================================================================
    ...
    _pEditMaskString.setMask = function (strMask)
    {
        ...
        for (i = 0; i < nLen ; i++)
        {
            ...
            {
                // MaskEdit에서 Escape에 진입할 경우.
                bEscape = true;
                continue;
            }
            else if (bEscape)
            {
                // MaskEdit에서 Escape를 사용할 경우.
                bEscape = false;
            }
    ...
    _pEditMaskString.inputText = function (txt)
    {
        ...
        if (nLeft != nRight)
        {
            //nLeft = this.clearBuffer(strText, strMaskedValue, nLeft, nRight);
        }
        else
        {
        ...
        return { newText: strText, newMaskedValue: strMaskedValue, end: nNewLeft };
    };
    ...
    _pEditMaskString.makeMaskedValue = function (strValue)
    {
        ...
        for (i = 0, j = 0 ; i < maskLen ; i++)
        {
            ...
            if (Mask != 0 && defs[c])
            {
                ...
                while (c && !Mask.test(c) && !(c == this._FillMaskChar) && !(c=="\u200e"));  // ltr mark
    ...
    if (nexacro.Browser == "IE")
    {
        if (nexacro.BrowserVersion == 10)
        {
            ...
            {
                //trace("IE10 _on_default_input_keyinput : " + elem.getElementValue());
                if (this._accept_keyinput_event === false)
                {
                    this._accept_keyinput_event = true;
        ...
        else if (nexacro.BrowserVersion >= 9)
        {
            _pEditMaskString._on_default_input_keyinput = function (elem)
            {
                //trace("IE9 _on_default_input_keyinput : " + elem.getElementValue());
                if (this._accept_keyinput_event === false)
                {
                    this._accept_keyinput_event = true;
        ...
        else
        {
            _pEditMaskString._on_default_input_keyinput = function (elem)
            {
                //trace("IE8 _on_default_input_keyinput : " + elem.getElementValue());
                if (this._accept_keyinput_event === false)
                {
                    this._accept_keyinput_event = true;
    ...
    //==============================================================================
    // nexacro.EditMaskNumber : mask type number use
    //==============================================================================
    ...
    _pEditMaskNumber._destroy = function ()
    {
        if (application._input_paste_comp == this.comp)
            application._input_paste_comp = null;
    ...
    _pEditMaskNumber.split = function (strText)
    {
        var nBegin = 0;
        var nSign;
        ...
        var nPoint = strText.indexOf(this._default_decimal_point, nBegin);
        ...
        return { "sign": nSign, "pointexist": bPoint, "numVal": strNumber, "decVal": strDecimal };
    };
    ...
    _pEditMaskNumber.clearBuffer2 = function (str, start, end)
    {
        ...
        else
        {
            ...
            if (nPoint < 0) nPoint = strPost.length;
            if (start <= nPoint)
            {
                strTemp = this.makeText(this.normalizeValue(strPost));
        ...
        return { "text": strPost, "end": end, "pstrDeleted": pstrDeleted };
    };

    _pEditMaskNumber.isDigit = function (c)
    {
        if (!this._digit) this._digit = /[0-9]/;
        if (this._digit.test(c)) return true;
        return false;
    };
    ...
    _pEditMaskNumber.findIndex = function (strSource, strTarget, nPos, bDeleteComma)
    {
        ...
        if (nSource < 0) nSource = strSource.length - 1;
        if (nTarget < 0) nTarget = strTarget.length - 1;
    ...
    _pEditMaskNumber.onInputDigit = function (strText, xchNum, nCaret)
    {
        ...
        //if (this._nLimitType != 0 && this._nLimitType != 1
		//	&& bPoint && (this._nDecimalMax >= 0) && (this._nDecimalMax < (strText.length - (nPoint + 1))))
        //{
        //	strText = strText.substring(0, nPoint + this._nDecimalMax + 1);
        //}
        ...
        return { "text": strText, "pos": nCaret };
    };
    ...
    _pEditMaskNumber._apply_sign_filter = function (c)
    {
        var text = this._text;
        var value = this._value ? this._value : "";
    ...
    _pEditMaskNumber._apply_comma_filter = function (c)
    {
        var value = this._value ? this._value : "";
        var text = this._text;
        var begin_pos = this._input_begin_pos;
    ...
    _pEditMaskNumber._apply_digit_filter = function (c)
    {
        var ret = this.onInputDigit(this._text, c, this._input_begin_pos);
        var text = ret.text;

        this._setValue(this.normalizeValue(text));
        this._setText(text);
        this.setElementCaretPos(ret.pos, ret.pos);
    };
    ...
    _pEditMaskNumber._fire_text_event = function (chartext)
    {
        ...
        else if (this._pasteAction)
        {
        	...
        	while (i < chartext.length)
        	{
        		...
        		{
        			this._apply_sign_filter(filterChar);
        		}
        		else if (this.isComma(filterChar))
        		{
        			this._apply_comma_filter(filterChar);
        		}
        		else if (this.isDigit(filterChar))
        		{
        			this._apply_digit_filter(filterChar);
        		}
        		i++;
        	}
        }
        ...
        {
            var filterChar = this._is_maskchar(chartext);
    ...
    //==============================================================================
    // nexacro.CompositionStatus : status info
    //==============================================================================
    ...
    _pEditBaseUndoStack.undoStack = [{ value: 0, selectionStart: 0, selectionEnd: 0 }];
    ...
    _pEditBaseUndoStack.push = function (value, text, start, end)
    {
        ...
        var undoItem = { value: value, text: text, selectionStart: start, selectionEnd: end };

        if (this.undoStack[this.undoPosition].value !== value)
        {
