...
if (nexacro.Component)
{
    ...
    // enabletouchevent=false 일 때 (isTap = false 일 때 focus 처리 없음, event fire 없음)
    _pComponent._on_touch_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, isTap)
    {
        ...
        if (isTap)
        {
            ret = this._on_bubble_touch_lbuttondown(elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, true, isTap); // user event bubble
        }
        ...
        if (!pThis.onlbuttondown || (pThis.onlbuttondown && !pThis.onlbuttondown.defaultprevented))
        {
            ...
            ret = this._on_bubble_touch_lbuttondown(elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, false, isTap); // system event bubble
        }
        return ret;
    };
    ...
    _pComponent.on_mouseleave_basic_action = function (is_subcontrol_bubble)
    {
        if (is_subcontrol_bubble)
        {
            ...
            else if (!this._isSelected()) // subcontrol 이 selected 된 경우 pseudo 변경을 무시
            {
                var tmp_comp = this;
                var alive = true;
            ...
            if (this.visible && this._isEnable())
            {
                ...
                if (event_bubbles !== true)
                {
                    if (!pThis.onmousewheel || (pThis.onmousewheel && !pThis.onmousewheel.defaultprevented))
                    {
                        ...
                        if (this.vscrollbar && this.vscrollbar.enable && this.vscrollbar.visible) // Associated RP_38605, 38302)
                        {
                            var vscrollbar = this.vscrollbar;
                            var old_vpos = vscrollbar._pos;
                            this._setVScrollDefaultAction(vscrollbar, wheelDeltaY);
    ...
    _pComponent._on_contextmenu = function (elem, event_bubbles, from_comp, from_refer_comp)
    {
        ...
        if (this._isEnable()) 
        {
            ...
            var listener = nexacro.Browser == "Safari" ? root_comp.onrbuttondown : root_comp.onrbuttonup; // [REQ_35042] 2014-05-28 safari : rbuttondown - contextmenu - rbuttonup, other browser : rbuttondown - rbuttonup - contextmenu
    ...
    _pComponent._on_contextmenu_default_action = function ()
    {
        ...
        {
            if (comp instanceof nexacro.Form) return true;
            return false;
        }
        else if (use_context_menu == "edit")
        {
            if (comp instanceof nexacro.Edit || comp instanceof nexacro.TextArea) return true;
                return false;
        }
        else
        {
    ...
    _pComponent._on_bubble_slide = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, event_bubbles, fire_comp, refer_comp, bScroll, is_userbubble)
    {
        ...
        else
        {
                ...
                (event_bubbles !== true) && this.parent)
            {
                ...
                {
                    //RP 43072 확대 시 스크롤이 3번째 올라가는 현상
                    //iOS 에서 스크롤 도달 로직 수정.(정확한 원인은 분석중..)
    ...
    _pComponent.on_fire_sys_onslide = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, from_comp, from_refer_comp)
    {
        ...
        else
        {
            ...
            // Check Vert
            if (ydeltavalue != 0 && this.vscrollbar && this.vscrollbar.enable &&
