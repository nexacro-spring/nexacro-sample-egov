...
if (nexacro.Browser != "Runtime")
{
    if (!nexacro.Element) 
    {
        ...
        nexacro.__setLastFocusedElement = function (elem)
        {
            ...
            if (elem)
            {
                var win = elem.linkedcontrol ? elem.linkedcontrol._getWindow() :
                    (elem._parent_elem ? elem._parent_elem.linkedcontrol._getWindow() : null);
                if (win)
                {
                    var root_win = win;
                    while (true)
	    ...
	    //==== internal handles =========//
	    _pElement._handle = null;      // HTML DOM Element Handle
	    _pElement._dest_handle = null; // HTML DOM Element Handle -  for append/remove child handle
        ...
        _pElement.setElementVisible = function (visible)
        {
            if (this.visible != visible)
            {
                ...
                if (_handle)
                {
                    ...
                    if (nexacro.OS == "Android" || nexacro.OS == "iOS")    //RP 40313
                    {
                        if (this.visible)
                        {
                            var v = this.zindex;
        ...
        _pElement.setElementRtlDirection = function (rtldirection)
        {
        	...
        	{
        		//trace("_pElement.setElementRtlDirection : " + this.rtldirection, rtldirection);
        		...
        		//this.setElementImageMirror(null, true);
        	}
        };
        ...
        //==============================================================================
	    // Control Element's Sub Elements
        //==============================================================================
        if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 8)
        { // IE6,7,8
            // for VML support
            //======================================================================
            // nexacro._VMLContainerElement (HTML ���� Element)
            //======================================================================
            ...
            __pVMLContainerElement.create = function (before_node)
            {
                ...
                if (_owner_elem._handle)
                {
                    ...
                    // update Contents - this operation should do after appendChildElement() && all update
                    this.updateVMLInfo();
                }
            };
            ...
            // move, resize & set position
            // no need Moving & Sizing Control - need for contents
            __pVMLContainerElement.setElementSize = function (width, height)
            {
                if (this.width != width || this.height != height)
                {
                    ...
                    // no need DOM size control
                    // update Contents
                    if (this.bordertype)
                    {
                        this.setVMLBorder(this.bordertype, this.border);
                        // apply Contents to DOM
                        if (this._handle)
                        {
                            this.updateVMLInfo();
                        }
            ...
            {
                if ((background && (this.background != background || this.background._syscolor != background._syscolor)) ||
                    (background.color == "@gradation" &&
                    (gradation && (this.gradation != gradation || this.gradation._sysvalue2 != gradation._sysvalue2))))
                {
                    if (background)
                    {
                        ...
                        else if (nexacro.BrowserVersion == 8) //VML IE8 Cursor Bug
                        {
                            background._syscolor = "transparent";
                            background._sysopacity = 0;
                            this._backgroundtype = 1;
                ...
                {   
                    ...
                    if (_need_update_background)
                    {
                        ...
                        else if (_backgroundtype == 1)
                        {
                            ...
                            else if (_applied_backgroundtype == 2) //do not use now..
                            {
                                var _fill_handle = this._background_fill_handle;
                                if (_fill_handle)
                                {
                                ...
                                _fill_handle = _doc.createElement("<v:fill color = '" + background._syscolor + "'" + " opacity= '" + (background._sysopacity / 100) + "' />"); //background opacity						   
                                _background_handle.appendChild(_fill_handle);
                                this._background_fill_handle = _fill_handle;
                            }
                        }
            ...
            // end of _RepeatImageElement - IE
            delete __pRepeatImageElement;


            //======================================================================
            // nexacro._StretchImageElement
            //======================================================================
            ...
            // end of _StretchImageElement - IE
            delete __pStretchImageElement;
        }


	    //======================================================================
	    // nexacro._QuadImageElement
	    //======================================================================
	    ...
	    { // IE7,8,9
	        ...
	        //==== defaule values =========//
	        __pEdgeImageElement._edge_type = 0; // no edge, 1: edge_x only, 2: edge_y only, 3: dual edge (x,y)
	        __pEdgeImageElement._applied_edge_type = 0; // no edge, 1: edge_x only, 2: edge_y only, 3: dual edge (x,y)
	        __pEdgeImageElement._img_url = "";
	        __pEdgeImageElement._applied_img_url = "";
	        ...
	        //==== internal handles =========//
	        __pEdgeImageElement._image_cc_node = null;
	        __pEdgeImageElement._div_lt_node = null;
	        __pEdgeImageElement._div_rt_node = null;
	        __pEdgeImageElement._div_lb_node = null;
	        __pEdgeImageElement._div_rb_node = null;
	        __pEdgeImageElement._image_tt_node = null;
	        __pEdgeImageElement._image_rr_node = null;
	        __pEdgeImageElement._image_bb_node = null;
	        __pEdgeImageElement._image_ll_node = null;
	        //==== default flags =========//
	        __pEdgeImageElement._is_nc_element = true;
	        __pEdgeImageElement._need_update_imagesize = false;
	        //==== end of initial =========//
	            ...
	            if (_handle)
	            {
	                ...
	                if (url_change)
	                {
	                    ...
	                    else
	                    {
	                        ...
	                        this._img_width = 0;
	                        this._img_height = 0;
	                        this._updateLayout(_handle, 0, width, height, 0, 0, 0, 0);
	                    }
	                }
	        ...
	        __pEdgeImageElement._createCorners = function (_doc, _handle, edgex, edgey)
	        {
	            ...
	            node_style.fontSize = "1px"; //div image size problem
	        ...
	        __pEdgeImageElement._updateLayout = function (_handle, edge_type, width, height, img_width, img_height, edgex, edgey)
	        {
	            ...
	            if (_handle)
	            {
	                ...
	                if (edge_type != 0)
	                {
	                    ...
	                    node_style.clip = nexacro._makeImageClipInfo(edgey + img_offset_y,
                                                                    width - edgex + img_offset_x,
                                                                    height - edgey + img_offset_y,
                                                                    edgex + img_offset_x);
	                }
	                // set current edge type
    ...
    // TextBoxElement._createTextElementHandle
	    if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 7)
	    {
	        _pTextBoxElement._createTextElementHandle = function (_doc, left, top, width, height)
	        {
	            ...
	            _cell_node.style.whiteSpace = "pre"; //nowrap으로하면 공백이 표현되지 않아 변경함
	    ...
	    { // HTML5 || IE8,9 ==> not IE7
	        _pTextBoxElement._createTextElementHandle = function (_doc, left, top, width, height)
	        {
	            ...
	            cell_style.whiteSpace = "pre"; //nowrap으로하면 공백이 표현되지 않아 변경함
	        ...
	        _pTextBoxElement.setElementSize = function (width, height)
	        {
	            if (this.width != width || this.height != height)
	            {
	                ...
	                if (_handle)
	                {
	                    _handle.style.textOverflow = "";            // IE8에서 ellipsis 설정이 된 상태에서 제대로 반영되지 않음.
	                    nexacro.__setDOMNodeStyleSize(_handle.style, width, height);
	    ...
	    _pTextBoxElement.create = function ()
	    {
	        ...
	        if (_owner_elem && _owner_elem._handle && !this._handle)
	        {
	            ...
	            // update Eleme's properties to DOM
	            if (!this.visible)
	            {
	                nexacro.__setDOMNodeStyleVisible(handle_style, false);
	            }
	    ...
	    _pTextBoxElement.setElementAlign = function (align)
	    {
	        ...
	        if (_cell_node)
	        {
	            _cell_node.style.textOverflow = "";            // IE8에서 ellipsis 설정이 된 상태에서 제대로 반영되지 않음. 
	    ...
	    if (nexacro.Browser == "IE" || nexacro.Browser == "Safari")
	    {
            //TextBox에서 IE9, Safari에서 \r\n 문자 표시문제
	    ...
	    else
	    {
	        _pTextBoxElement.setElementText = function (text)
	        {
	            if (this.text !== text)
	            {
	                ...
	                {
	                    //wordwrap != "none"일때, textcontent를 사용하는 Browser에서 개행문자 표시문제 
	                    this.text = text.replace(/\r\n|\r|\n/g, "\r\n");
	                }
	    ...
	    _pTextBoxElement.setElementLineSpace = function (linespace)
	    {
		    if (this.linespace != linespace)
		    {
			    ...
			    if (_cell_node)
			    {
                    ...
                    if (comp)
                    {
                        var font_size = nexacro._getTextSize2("Wj", this.font || this._getParentFont());
	    ...
	    _pImageElement.create = function ()
	    {
		    ...
		    if (_owner_elem && _owner_elem._handle && !this._handle)
		    {
                ...
                if (nexacro.Browser == "IE")
                    nexacro.__setDOMNodeStyleAbsolute(_image_handle_style);      //(fixed + absolute) position problem with Android browser
	    ...
	    {
	        var elem = this; // [REQ_36181] 2014-05-28 async loading order revise
	    ...
	    //======================================================================
	    // nexacro.AlignImageElement - this Element
        ...
        {
            nexacro.__setDOMNodeAccessibilityRole(this._input_handle, this._parent_elem.accessibility_role);
        };
        ...
        else if (nexacro.Browser == "Gecko" || nexacro.Browser == "Chrome")
        {
            _pInputElement._wantAccessibilityAdditionalLabel = function ()
            {
                return true;
		...
		if (nexacro.Browser == "IE" && nexacro.BrowserVersion < 9)
		{
		    _pInputElement._createInputHandle = function ()
		    {
		        ...
		        if (elem_handle)
		        {
		            ...
		            if (this.halign && this.valign)
		            {
		                var txt_size = nexacro._getTextSize2("Wj", this.font || this._getParentFont());
		                align_style_text = nexacro.__getInputHTMLStyleAlignXY(this.halign, this.valign, this.height, txt_size[1]);
		            }

		            var input_attr_str =
                        nexacro.__getHTMLAttrEnable(this.enable) +
		            ...
		            var input_style_str =
                        nexacro.__getHTMLStyleAbsoluteTransparent() +
		            ...
		            var type_str = " type='" + (this.password ? "password" : this._type) + "'";
		            var value_str = " value='" + (this.text ? nexacro._encodeXml(this.text) : "") + "'";
		            var id_str = " id='" + this.parent._handle.id + "_input'";
		            var html = "<input" + type_str + id_str + input_attr_str + value_str + " style='" + input_style_str + "'/>";

		            // update node's contents && attach it
		...
		else
		{
		    _pInputElement._createInputHandle = function ()
		    {
		        ...
		        if (elem_handle)
		        {
                    ...
                    input_handle._linked_element = this;
		            input_handle.type = this.password ? "password" : this._type;
        ...
        _pInputElement._createPlaceHolderElement = function (visible)
        {
            ...
            if (this._handle && !display_elem)
            {
                ...
                display_elem.setElementColor(this.color);
        ...
        _pInputElement.create = function ()
        {
            ...
            if (_owner_elem && _owner_elem._handle)
            {
                ...
                if (!_handle)
                {
                    ...
                    // IE8 이하에서 parent div에 cursor 설정해야 함.
	    ...
	    _pInputElement._updateInputValue = function ()
	    {
	        ...
	        else if (!active && nulltext && nulltext.length > 0)
	        {
	            ...
	            {
	                // IE8에서 Grid에서 row 이동시 empty string 입력 후 text 변경시 DOM이 갱신되지않는 문제 처리
	    ...
	    if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 8)
	    {
	        _pInputElement.setElementPassword = function (is_password_type)
	        {
	            if (this.password != is_password_type)
	            {
	                this.password = is_password_type;

	                var input_handle = this._input_handle;
	                if (input_handle)
	                {
	                    var new_handle = nexacro.__changeInputDOMNodeType(input_handle, is_password_type ? "password" : this._type);
	    ...
	    else
	    {
	        _pInputElement.setElementPassword = function (is_password_type)
	        {
	            if (this.password != is_password_type)
	            {
	                ...
	                if (input_handle)
	                {
	                    nexacro.__changeInputDOMNodeType(input_handle, is_password_type ? "password" : this._type);
	                }
	            }
	        };
	    }
	    ...
	    // ["none","alpha","alpha,full","hangul","hangul,full","katakana","katakana,full","hirakana","direct"]
	    ...
	    {
	        //Global Attributes 
	        if (v == "accesskey") return "accessKey";
	        else if (v == "contenteditable") return "contentEditable";
	        else if (v == "tabindex") return "tabIndex";
	        else return v;
	    };
	    ...
	    if (nexacro.Browser == "IE")
	    {
	        ...
	        _pInputElement._inputhandler_onkeyinput_forward = function (evt)
	        {
	        	...
	        	// IE11은 oninput이 발생하기 때문에 propertyName이 undefined임.
            ...
            // IE6는 ime입력시 keydown보다 input이 먼저 발생하는 문제가 있음.
	        // winxp의 IE는 모두 그러하다.
	        if (nexacro.OSVersion >= 6.0) // NT6.0 : Vista
	        {
	            _pInputElement._on_sys_keyinput = function (keycode, altkey, ctrlkey, shiftkey)
	            {
	                var comp = this._parent_elem.linkedcontrol;
	                if (this._isPreventDefault(comp, "onkeydown")) return;
	        ...
	        else
	        {
	            _pInputElement._on_sys_keyinput = function (keycode, altkey, ctrlkey, shiftkey)
	            {
	                ...
	                if (this._accept_keyinput || this._is_keydown)
	                {
	                	//select후 ascii문자 입력시 첫 문자 입력 안되는 버그.
	        ...
	        _pInputElement._on_sys_paste = function (keycode, altkey, ctrlkey, shiftkey)
	        {
	            var comp = this._parent_elem.linkedcontrol;
	            if (this._isPreventDefault(comp, "onkeydown")) return;
	            if (!comp._is_alive) return;
	        ...
	        _pInputElement._on_sys_cut = function (keycode, altkey, ctrlkey, shiftkey)
	        {
	            var comp = this._parent_elem.linkedcontrol;
	            if (this._isPreventDefault(comp, "onkeydown")) return;
	            if (!comp._is_alive) return;
	    ...
	    else
	    {
	        ...
	        {
	            application._is_input_paste = true;
	            if (application._com_waiting) nexacro._stopSysEvent(evt);
	    ...
	    _pInputElement._on_sys_focus = function (target)
	    {
	        ...
	        if (this._isPreventDefault(comp, "onfocus")) return;
	        if (!comp._is_alive) return;
	    ...
	    _pInputElement._on_sys_blur = function (target)
	    {
	        ...
	        if (this._isPreventDefault(comp, "onkillfocus")) return;
	        if (!comp._is_alive) return;
	        ...
	        // display null text 처리 및 focus flag처리를 여기로 옮김.
	        var input_handle = this._input_handle;
	        if (input_handle && this._is_focused)
	        {
	        ...
	        if (win_handle != win_handle.parent)
	        {
	            ...
	            if (!nexacro._enableaccessibility && nexacro.Browser == "IE" && nexacro.BrowserVersion > 9)
	            {
	                //if (!last_focus._input_element)
	    ...
	    _pInputElement._on_sys_keypress = function (keycode, charcode, altkey, ctrlkey, shiftkey, evt)
	    {
	        ...
	        if (this._isPreventDefault(comp, "onkeydown")) return;
	        if (!comp._is_alive) return;
	        if (ctrlkey && charcode == 118) return;
	    ...
	    if (nexacro.Browser != "IE" || (nexacro.Browser == "IE" && nexacro.OSVersion >= 6.0))
	    {
	        if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 8)
	        {
                ...
                _pInputElement._on_sys_keyup = function (keycode, altkey, ctrlkey, shiftkey)
                {
                    ...
                    if (!this._parent_elem) return;

                    var comp = this._parent_elem.linkedcontrol;
                    if (this._isPreventDefault(comp, "onkeyup")) return;
                    if (!comp._is_alive) return;
	    ...
	    else
        {
	        ...
	        _pInputElement._on_sys_keydown = function (keycode, altkey, ctrlkey, shiftkey)
	        {
	            ...
	            // alt키를 눌러 윈도우 메뉴로 포커스를 이동시킨 후 IME입력을 막아야 함.
	    ...
	    if (nexacro.Browser == "IE")
	    {
	        _pTextAreaElement._bindEvent = function ()
	        {
				...
				//InputElement와 동일한 이벤트는 InputElement에 추가
	        	nexacro.InputElement.prototype._bindEvent.call(this);

	        	//TextAreaElement에서만 사용되는 이벤트를 별도로 추가.
	            ...
	            if (container)
	            {
                    ...
                    if ((_win && _win._cur_ldown_elem) ||
                           (nexacro.isTouchInteraction &&
                            (_win && _win._cur_touch_elem || ((pos && pos != -1) && pos.begin != pos.end))))
                    {                        
                        var elem_scroll_top = container.parent.scroll_top;
                        var elem_scroll_left = container.parent.scroll_left;
	    ...
	    if (nexacro.Browser == "IE" && nexacro.BrowserVersion < 9)
	    {
	        _pTextAreaElement._createInputHandle = function ()
	        {
	            ...
	            if (elem_handle)
	            {
	                ...
	                var input_attr_str =
                        nexacro.__getTextAreaHTMLAttrEnable(this.enable) +
                        nexacro.__getHTMLAttrReadOnly(this.readonly) +
                        nexacro.__getHTMLAttrTabIndex(this.tabindex) +
	    ...
	    _pTextAreaElement.setElementSize = function (width, height)
	    {
	        if (this.width != width || this.height != height)
	        {
	            ...
	            if (_handle)
	            {
	                ...
	                // firefox의 scrollbar는 client 크기가 scrollbar 사이즈보다 적으면 scrollbar를 표시하지 않는다.
	    ...
	    _pTextAreaElement.setElementAlignXY = function (halign, valign)
	    {
	        ...
	        if (input_handle)
	        {
	            var text_height = this.getScrollHeight();
	            var blank_height = Math.max(0, this.height - text_height);
	            nexacro.__setTextAreaDOMNodeStyleAlignXY(input_handle.style, _halign, valign, blank_height);
	        }
	        var _elem = this._display_elem;
	        if (_elem)
	        {
	    ...
	    _pTextAreaElement.setElementLineSpace = function (linespace)
	    {
	        ...
	        if (input_handle)
	        {
	            ...
	            //on_created_contents에서 호출되면 안되기 때문에 밖으로 빼냄.
	        }
	    };
        ...
        nexacro._inputhandler_onmouseup = function (node, evt)
	    {
	        ...
	        if (elem && elem._on_sys_mouseup)
	        {
	        	...
	        	if (nexacro.Browser != "IE" && nexacro._getSysEventBtnCode(evt) == (application._is_attach_childframe ? nexacro_HTMLSysEvent[id].MOUSE_LBUTTON : nexacro_HTMLSysEvent.MOUSE_LBUTTON))
                {
	        	    evt.id = id;
        ...
        nexacro._inputhandler_onmousemove = function (node, evt)
	    {
	        ...
	        {
	            if (nexacro.Browser != "IE" && nexacro._getSysEventBtnCode(evt) == nexacro_HTMLSysEvent.MOUSE_LBUTTON)
                {
	                nexacro._stopPropagation(evt);
                }
        ...
        //======================================================================
        // nexacro.ControlElementBase - this ControlElementBase has container or element
        //======================================================================
	    ...
	    _pControlElementBase._accessibility_stat_checked = undefined; // undefined : this element does not support being checked.
	    _pControlElementBase._accessibility_stat_pressed = undefined;
	    _pControlElementBase._accessibility_stat_selected = undefined;
	    _pControlElementBase._accessibility_stat_expanded = undefined; // undefined : neither expandable nor collapsible.
	    ...
	    _pControlElementBase._background_style = 0; // 0: DOM's background, 1: image(no-repeat, repeat), 2: stretch, 3: quad, 4: edge
    ...
    { // IE6,7,8
        ...
        //======================================================================
        // _pControlElementBase's Platform Methods (HTML Element)
        //======================================================================
        ...
        _pControlElementBase._setControlOpacity = function (element, _handle, sysopacity)
        {
            ...
            if (this._node_opacity_str != filter_str)
            {
                ...
                {
                    var handle_style = _handle.style;
                    nexacro.__setDOMNodeStyleForceVisibility(handle_style, true); //IE Darkbug - Filter + scroll
                    if (filter_str)
                    {
        ...
        { // IE6 || quirksMode
            _pControlElementBase._setControlSize = function (element, _handle, width, height)
            {
                ...
                if (this._inner_width != inner_width || this._inner_height != inner_height)
                {
                    ...
                    // update background pos/width/height
        ...
        { // IE6 || quirksMode
            ...
            {
                // this function called by ControlElement ==> _handle._element_type == 3
                ...
                var usevml = application.usevml; //vml flag
                ...
                if (usevml && round)
                { // for round border -- this type is vmlType	                
        ...
        { // for IE6,7,8 ==> not allow stretch, quad, edge
            if (_handle)
            {
                var handle_style = _handle ? _handle.style : null;
        ...
        { // for IE6,7,8 ==> not allow stretch, quad, edge
            ...
            if (_handle)
            {
                ...
                if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 8)
                { //IE8 Bug - if Background Image repeat type == Stretch                     
            ...
            {
                if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 8)
                {   //IE에서 background-color가 transparent일때 event 통과 문제	                            
                    if (this.linkedcontrol._is_subcontrol == true && syscolor == "transparent" && url == "")
                         url = null;
                }
                ...
                else
                {
                    if (bkimgurl)
                    {
                        ...
                        else
                        {
                            ...
                            if (!bkrepeat) bkrepeat = "no-repeat";
                            if (bkgradation)
                            {
                                this._setDOMBackgroundOnly(_handle, bkcolor, bkgradation);
        ...
        {
            // _background_style == 1 : repeat && no-repeat type
        ...
        {
            // _background_style == 2 : stretch type
    ...
    { // HTML5 || IE9
        //======================================================================
        // ControlElement's Platform Methods (HTML ���� Element)
        //======================================================================
        ...
        _pControlElementBase._setControlBorder = function (element, _handle, border, bordertype)
        {
            ...
            {
                if (bordertype)
                    nexacro.__setDOMNodeStyleBorderType(_handle.style, bordertype);
                else
            ...
            if (border && !border._is_real_empty())
            {
                // for round border -- draw with by VML color -- not reach this
                // apply border to DOM
                if (_handle)
                {
                    var handle_style = _handle.style;
        ...
        //======================================================================
        _pControlElementBase._setDOMBackgroundOnly = function (_handle, syscolor, sysgradation)
        { // for HTML5 || IE9 ==> not allow quad, edge
            syscolor = (syscolor ? syscolor : "transparent");
            if (_handle)
            {
                var handle_style = _handle.style;
    ...
    else
    {
        ...
        //======================================================================
        // nexacro.SimpleControlElement - this SimpleControlElement has no container
        //======================================================================
	    ...
	    _pSimpleControlElement.create = function ()
	    {
		    ...
		    if (_owner_elem && _owner_elem._handle && !this._handle)
		    {
			    ...
			    // create Border & Background SubElements
			    ...
			    // child_elem is creating by Component
		    }
	    };
        ...
        _pSimpleControlElement.destroy = function ()
        {
            ...
            if (_handle)
            {
			    ...
			    // client_element is destroyed by Component
	    ...
	    //////////////////////////////////////////////////////////////////////////
	    // SimpleControlElement == Container : next Method's == Conntainer's Method
        //////////////////////////////////////////////////////////////////////////
	    ...
	    //////////////////////////////////////////////////////////////////////////
	    // SimpleControlElement == Container : for Scroll ==> no NC AREA : scroll only
	    ...
	    //======================================================================
	    // nexacro.ControlElement - this Element has Border && background && Container
	    //======================================================================
	    ...
	    {
            //nexacro.ControlElementBase.call(this, parent_elem);
		    this.parent = parent_elem;
		    this._parent_elem = parent_elem;
	    ...
	    _pControlElement.create = function ()
	    {
	        ...
	        if (_owner_elem && _owner_elem._handle)
	        {
	            if (!this._handle)
	            {
	                ...
	                //nexacro.__setDOMNodeToolTip(_handle, "");
        ...
        _pControlElement.destroy = function ()
        {
            ...
            if (_handle)
            {
			    ...
			    // destroy destroy Border & Background SubElements
			    this._destroyControlSubElements();
	    ...
	    _pFrameControlElement.create = function ()
	    {
	        if (!this._handle)
	        {
	            if (this._parent_elem == null)
	            {
	                ...
	                this.left = 0;
	                this.top = 0;
	                this.width = this._node_width = _win.clientWidth; // 이렇게 하면 mainframe node 크기가 맞질 않음 .. (border가 있는 경우)
	                ...
	                // props are already setted to client_element
	                // client_element is created by Component
	            }
	            else
	            {
	                // parent is always control element
	                ...
	                if (_owner_elem && _owner_elem._handle && !this._handle)
	                {
	                    ...
	                    if (_owner_elem instanceof nexacro.ModalOverlayElement) // [REQ35522] 2014-05-28 showmodal fixed
	                    {
        ...
        _pFrameControlElement.destroy = function ()
        {
            ...
            if (_handle)
            {
                ...
                if (this._client_element)
                    this._client_element.destroy();
			    // _client_element is destroyed by component
			    this._client_element = null;
            ...
            if (this._handle)
            {
                ...
                for (var i = 0; i < 8; i++)
                {
                    ...
                    if (resizable)
                        ...
                        cursor = new nexacro.Style_value("arrow"); // TODO check default로 되돌릴 방법??
        ...
        //======================================================================
        // nexacro.FrameResizeBorderElement
        //======================================================================
        ...
        {
            // overlay color가 완전 투명색일때 일부 브라우저에서는 hittest시 통과처리됨
            if (!color)
                return;
        ...
        nexacro.ScrollableControlElement = function (parent_elem)
        {
            ...
            // Transition & Transform ScrollableContainerElement를 쓰는 경우
            //var client_element = new nexacro.ScrollableContainerElement2(this);
	    ...
	    _pScrollableControlElement._scroll_showtype = -1; // 0 : auto, 1 : fix, 2 : overlap
        ...
        _pScrollableControlElement._updateClientSize = function ()
	    {
		    ...
		    if (scroll_showtype > 0)
		    {
                ...
                else if (scroll_showtype == 31) // alwaysvert
				{
                    if (this._vscroll_control)
				    {
					    vscroll_visible = true;
				...
				else if (scroll_showtype == 32) // alwayshorz
				{
                    if (this._vscroll_control && container_maxheight > zclient_height)
				    {
					    vscroll_visible = true;
		        ...
		        // zoom 처리되는 경우 client_width는 zclient_width값을 갖고 있지 않기때문에
                // client_element의 width 크기와도 비교해봐야 한다. (상황에 따라 안맞을수 있음)
			    ...
			    if (this._vscroll_control)
			    {
				    ...
				    else
				    {
					    if (this._vscroll_left != (client_left + client_width) || this._vscroll_top != client_top || this._vscroll_height != client_height || reset_vlimit)
					    {
                            ...
                            // Visibility=hidden인 경우 DOM의 스크롤 영역 계산시 포함되므로 Client영역 밖에 스크롤을 붙이면 안된다.
        ...
        //======================================================================
        // this ScrollableContainerElement has No Event Handlers -- Container only
	    ...
	    //==== internal handles =========//
	    //==== default flags =========//
	    //_pScrollableContainerElement._is_nc_element = true;
	    //==== end of initial =========//
        ...
        //======================================================================
        // nexacro.ScrollableInnerContainerElement
        //======================================================================
        // this ScrollableInnerContainerElement has inner Container(<div>) for css transform (HTML only)
            ...
            _pScrollableInnerContainerElement._syshandler_onscroll_forward = function (evt)
            {
                ...
                if (container)
                {
                    if (target.scrollLeft > 0)
                        target.scrollLeft = 0;  // translate 사용시엔 0으로 유지.
        ...
        //======================================================================
        // nexacro.PopupControlElement - this PopupControlElement has no container
        //======================================================================
        ...
        //==== internal handles =========//
        _pPopupControlElement._doc = null;         // Document Handle == HTML Document
        ...
        //======================================================================
        // nexacro.PopupScrollableControlElement - this Element is scrollable ControlElement
        //======================================================================
        // this PopupScrollableControlElement has ScrollControlElement
        ...
        //==== internal handles =========//
        _pPopupScrollableControlElement._doc = null;         // Document Handle == HTML Document
	    ...
	    //==============================================================================
	    // nexacro.TextElement : nexacro Element's basic flags & Utility Functions
	    // TextElement's DOM size control is always  fit by textsize
	    // move text by padding & align
	    //==============================================================================	
        ...
        if (nexacro.Browser == "IE")
        {
            _pPluginElement._createObjectElementHandle = function (_doc, left, top, width, height, params)
            {
                ...
                if (nexacro.BrowserVersion >= 9) // version 9 이상만 사용
                {
        ...
        _pPluginElement.callMethod = function ()
        {
            ...
            if (this._handle && this._object_node)
            {
                ...
                if (this._object_id)
                {
                    context = this._object_id;
                }
                else if (this._embed_node)
                {
                    context = this._embed_node;
                }
                else
                {
                    context = this._object_node;
                }
                return context[fn_name].apply(context, arguments);
            }
        };
...
//        _pPluginElement.on_update_position = nexacro._emptyFn; 
        ...
        _pPluginObject.callMethod = function ()
        {
            ...
            {               
                var fn_name = Array.prototype.shift.call(arguments);
                var value = this._handle[fn_name].apply(this._handle, arguments);
        ...
        //==== internal handles =========//
         ...
         _pWebBrowserPluginElement._embed_node = null; // for embed tag
         _pWebBrowserPluginElement._params = null;  // for Param tag    
         _pWebBrowserPluginElement._events = null;
         _pWebBrowserPluginElement._window = null;
         _pWebBrowserPluginElement._document = null;
         _pWebBrowserPluginElement._block_node = null; //for iframe enable 
        ...
        _pWebBrowserPluginElement.create = function ()
        {
            ...
            if (_owner_elem && _owner_elem._handle && !this._handle)
            {
                ...
                if ((nexacro.Browser == "IE" && nexacro.BrowserVersion >= 9) || (nexacro.isTouchInteraction && nexacro.SupportTouch))
                {
        ...
        _pWebBrowserPluginElement._createInternalFrameElementHandle = function (_doc, left, top, width, height, params)
        {
            ...
            _handle._linked_element = this;
            _handle._element_type = 8;
            _handle.frameBorder = 0; //webborwser don't use frameborder 
        ...
        _pWebBrowserPluginElement.setElementCodebase = function (codebaseurl) // 삭제 ?
        {
        ...
        //======================================================================
        // nexacro.MediaPlayerPluginElement
        //======================================================================
        ...
        else
        {
            ...
            _pMediaPlayerPluginElement._createObjectElementHandle = function (_doc, left, top, width, height, params)
            {
                ...
                //if (_handle.canPlayType(type + "; codecs=''") == "maybe")
                //{
                ...
                //nexacro._observeSysEvent(_handle, "abort", "onabort", this._media_eventhandler_abort);
                //nexacro._observeSysEvent(_handle, "canplay", "oncanplay", this._media_eventhandler_canplay);
                nexacro._observeSysEvent(_handle, "canplaythrough", "oncanplaythrough", this._media_eventhandler_canplaythrough);
                //nexacro._observeSysEvent(_handle, "durationchange", "ondurationchange", this._media_eventhandler_durationchange);

                //nexacro._observeSysEvent(_handle, "emptied", "onemptied", this._media_eventhandler_emptied);
                ...
                //nexacro._observeSysEvent(_handle, "loadeddata", "onloadeddata", this._media_eventhandler_loadeddata);
                //nexacro._observeSysEvent(_handle, "loadedmetadata", "onloadedmetadata", this._media_eventhandler_loadedmetadata);
                //nexacro._observeSysEvent(_handle, "loadstart", "onloadstart", this._media_eventhandler_loadstart);
                ...
                //nexacro._observeSysEvent(_handle, "playing", "onplaying", this._media_eventhandler_playing);
                nexacro._observeSysEvent(_handle, "progress", "onprogress", this._media_eventhandler_progress);
                //nexacro._observeSysEvent(_handle, "ratechange", "onratechange", this._media_eventhandler_ratechange);

                //nexacro._observeSysEvent(_handle, "seeked", "onseeked", this._media_eventhandler_seeked);
                //nexacro._observeSysEvent(_handle, "seeking", "onseeking", this._media_eventhandler_seeking);
                //nexacro._observeSysEvent(_handle, "stalled", "onstalled", this._media_eventhandler_stalled);
                //nexacro._observeSysEvent(_handle, "suspend", "onsuspend", this._media_eventhandler_suspend);

                nexacro._observeSysEvent(_handle, "timeupdate", "ontimeupdate", this._media_eventhandler_timeupdate);
                //nexacro._observeSysEvent(_handle, "volumechange", "onvolumechange", this._media_eventhandler_volumechange);
                //nexacro._observeSysEvent(_handle, "waiting", "onwaiting", this._media_eventhandler_waiting);

                this._object_node = _handle;
                return _handle;
            };

            _pMediaPlayerPluginElement._media_eventhandler_changedstatus = function (evt) // playstatuschanged
            {
                var comp = evt.target._linked_element._parent_elem.linkedcontrol;
                comp && comp._on_statuschanged(evt.type);
            };

            _pMediaPlayerPluginElement._media_eventhandler_timeupdate = function (evt) // currenttimechanged
            {
                var node = evt.target;
            ...
            _pMediaPlayerPluginElement._media_eventhandler_error = function (evt) // error
            {
                var node = evt.target;
                var comp = node._linked_element._parent_elem.linkedcontrol;
                if (comp)
            ...
            _pMediaPlayerPluginElement._media_eventhandler_canplaythrough = function (evt) // get size, get duration
            {
                var node = evt.target;
                var comp = node._linked_element._parent_elem.linkedcontrol;
        ...
        _pCanvasElement.drawArc = function (x, y, r, start_deg, end_deg, bClockwise, bNormal)
        {
            if (!bNormal)
            {
                trace("1 =>" + start_deg + " : " + Math.PI);

                trace("2 =>" + end_deg + " : " + Math.PI);
        ...
        if (!(nexacro.Browser == "IE" && nexacro.BrowserVersion < 9)) //supported HTML5
        {
            _pCanvasElement.create = function ()
            {
                ...
                if (_owner_elem && _owner_elem._handle)
                {
                    ...
                    if (!_handle)
                    {
                        ...
                        _handle = _doc.createElement("div");
                        _handle._linked_element = this;
                        _handle._element_type = 7;          // CHECK 뭘까??
        ...
        else 
        {
            ...
            _pCanvasElement._circle = function (x, y, r)
            {
                ...
                if (_handle)
                {
                    ...
                    _handle._pathArray.push({ x: p.x - _r, y: p.y - _r });
                    _handle._pathArray.push({ x: p.x + _r, y: p.y + _r });
                    _handle._pathArray.push({ x: sp.x, y: sp.y });
            ...
            _pCanvasElement.drawStroke = function ()
            {
                ...
                if (_handle && _handle._path_str) 
                {
                    ...
                    if (_handle._stroke_alpha != 1) vml_str += "opacity='" + _handle._stroke_alpha + "' ";
                    ...
                    _handle._draw_node.innerHTML = _handle._vml_str; //Draw VML 
                    _handle._path_str = "";
                }
            };
            //draw VML
        ...
        _pGridScrollableControlElement._updateClientSize = function ()
        {
            ...
            if (this._scroll_showtype > 0)
            {
                var v_elements = this._target_vscroll_elements,
				    v_element = v_elements,
				    h_elements = this._target_hscroll_elements,
				    h_element = h_elements;
                ...
                else if (scroll_showtype == 32) // alwayshorz
                {
                    if (this._vscroll_control && maxheight > v_client_height)
                    {
                        vscroll_visible = true;
                        client_width -= this._vscroll_width;
                        vscroll_limit = maxheight + this._hscroll_height - v_client_height; // 위의 로직은 휠영역 계산이 잘 못됨.
                    }
                    if (step_count > 0 && step_containers)
                    {
                        maxwidth = step_count * this.client_width;
        ...
        if (nexacro.OS == "Android" && (nexacro.Browser == "Safari" || nexacro.Browser == "MobileSafari"))   // Android Translate사용시 scrollbar trackbar이용시 trackbar draw가 안되는 경우가 있어 분기.
        {
        ...
        _pGridScrollableControlElement.setElementScrollMaxSize = function (width, height)
        {
            ...
            {
                if (maxheight != height) this.setContainerMaxHeight(height);
                if (maxwidth != width) this.setContainerMaxWidth(width);
                retn = true;
            }
        ...
        nexacro.GridBandContainerElement = function (parent_elem)
        {
            ...
            this._ext_inner = []; //scroll area extend
        };
        ...
        * position 이동  : 터치시 화면반응속도 중간, 스크롤속도 중간, 화면출렁거림 거의없음 (노드량이 많으면 느려질 수 있음)
        ...
        var _gridScrollMode = 1;        // 0: scrollTop, 1: inner translateY, 2: inner position
        ...
        if (_gridScrollMode)
        {
            ...
            _pGridBandContainerElement._refreshDOM = function ()
            {
                if (this._handle)
                {
                    nexacro.__setDOMNodeVScrollPos(this._handle, 1);   // onscroll에서 0으로 유지됨
                }
            }
        }
        else  // scrollTop 사용
        ...
        //if (_gridScrollMode && _gridScrollMode != 2)
        ...
        var _pGridAreaContainerElement = nexacro._createPrototype(nexacro.ScrollableInnerContainerElement, nexacro.GridAreaContainerElement);
        ...
        _pGridAreaContainerElement._refreshDOM = function ()
        {
            if (this._handle)
            {
                ...
                nexacro.__setDOMNodeHScrollPos(this._handle, 1);   // onscroll에서 0으로 유지됨
            }
        }
        ...
        //======================================================================
        // nexacro.GridCellControlElement - this Element has Border && background && Container
        //======================================================================
        // this GridCellControlElement has No Event Handlers -- Container only
        // this GridCellControlElement has Border
        nexacro.GridCellControlElement = function (parent_elem, area, mode, padding)
        {
            ...
            this._mode = "" // (expandtext, text, normal)
        ...
        //======================================================================
        // nexacro.GridCellTextContainerElement
        //======================================================================
        ...
        //======================================================================
        // nexacro.GridCellTextSimpleContainerElement
        //======================================================================
        ...
        { // HTML5 || IE8,9 ==> not IE7
            ...
            _pGridCellTextSimpleContainerElement._createTextElementHandle = function (_doc, left, top, width, height)
            {
                ...
                height = this.parent.height;    // simple에선 무조건 부모사이즈로

                nexacro.__setDOMNodeStyleSize(handle_style, width, height);
                handle_style.display = "table-cell";
                handle_style.whiteSpace = "pre"; //nowrap으로하면 공백이 표현되지 않아 변경함
        ...
        //======================================================================
        // nexacro.EventPassOverlayElement - Event Pass & Display Text
        //======================================================================
        ...
        {            
            ...
            /* property for grid hscroll*/
            this._start_hscroll = 0;
            
            /* pointer event pass property */
            ...
            {                
                ...
                {                    
                    ...
                    for (var i = 0; i < len; i++)
                    {
                        ...
                        /* overlayelement 내부에서 leave/enter event 미발생문제 */
        ...
        _pEventPassOverlayElement.updateCellNodeClient = function (left, top, width, height, cell_hpos, is_fake_merge)
        {
            ...
            if (_cell_node)
            {
                ...
                {
                    left = 0;
                    if (cell_hpos >= this._start_hscroll)
                    {
                        cell_hpos -= this._start_hscroll;
                        left = -cell_hpos;
                        width += cell_hpos;
                    }
                }
        ...
        _pEventPassOverlayElement._getNodeAlignPos = function ()
        {
            ...
            if (_cell_node)
            {
                ...
                var tx_size = nexacro._getTextSize(this.text, this.font, true, this.width, this.wordwrap);
                
                if (this.height > 0 && this.height > tx_size[1])
                {
                    if (valign == "bottom")
                    {
                        tx_size = nexacro._getTextSize(this.text, this.font, true);
        ...
        _pEventPassOverlayElement.setElementStartHScrollPos = function (left)
        {
            this._start_hscroll = left;
        };
