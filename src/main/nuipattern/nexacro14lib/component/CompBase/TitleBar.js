...
if (!nexacro.TitleBar)
{
    ...
    nexacro.TitleBar = function(id, position, left, top, width, height, right, bottom, parent)
    {
        ...
        /* internal variable */
        this._state_openstatus = 0; //normal(0), restore(1), minimize(2), maximize(3)7
    ...
    _pTitleBar.on_apply_custom_pseudo = function (pseudo)
    {
        ...
        if (curstyle.gradation != gradation)
        {
    ...
    _pTitleBar.on_apply_style_icon = function (icon)
    {
        if (this._icon_elem)
        {
            // OwnerFrame에 icon이 설정되어있으면 그쪽이 우선시된다. 그렇지 않으면 Titlebar의 icon값
    ...
    _pTitleBar._updateControlPosition = function (width, height)
    {
        ...
        // TODO icon크기 얻을 방법.. Element 교체 (Image Control)
        // TODO text 크기 다양성 고려 안되어있음
        // TODO 세로텍스트 (현재 지원 불가)

        // [10px][icon][6px][titletext][?px][minbtn][1px][maxbtn][1px][closebtn][2px]
        var iconleftgap = 10;
        var iconrightgap = 6;
        var buttonwidth = 22;
        var buttonheight = 20;
    ...
    _pTitleBar.on_apply_prop_rtldirection = function ()
    {
    	...
    	trace("_pTitleBar.on_apply_prop_rtldirection : " + this._rtldirection + this.parent.name);
