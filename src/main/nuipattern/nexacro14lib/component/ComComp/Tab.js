...
if (!nexacro.TabComponent) 
{
    nexacro.TabComponent = function (id, position, left, top, width, height, right, bottom, parent)
	{
        ...
        this._accessibility_role = "tab"; //aria에서는 버튼이 tab이고 버튼들을 가지고 있는것이 tablist임.(하지만 aria에서 tabpage가 tablist의 자식이아님)
	};
	...
	_pTabComponent._rearrangeBorder = function () 
	{
	    ...
	    //var rc = this._buttonRect[this.tabindex];
		var rc;
	...
	_pTabComponent._rearrangeTopButton = function () 
	{
        ...
        if ((total_width - bmright) >= maxWidth)
		{
			...
			else 
			{
                ...
                var rcButton = { left: 0, top: 0, right: 0, bottom: 0 };
        ...
        var pagerect = { left: 0, top: iMaxBottom, right: clientWidth, bottom: clientHeight };
		var btnrect = null;
	...
	_pTabComponent._drawClientTopBottom = function () 
	{
		...
		else
		{
			...
			for (var i = tabpagecnt - 1; i >= start; i--)
			{
				...
				else if (i == this._scrollIndex) //최소 한개는 나와야 함.
				{
				    this._lastIndex = end = i;
				    break;
				}
	...
	_pTabComponent._drawClientLeftRight = function ()	
	{
		...
		else
		{
            ...
            /*
			if ((buttonRect[this.tabindex].bottom - buttonRect[this._scrollIndex].top + bmtop) <= tabs_height &&
				(buttonRect[this.tabindex].top - buttonRect[this._scrollIndex].top) >= 0)
			{
				this._drawTabButton(this.tabindex);
			}
            */
	...
	{
        var btnrect = { left: 0, top: 0, right: 0, bottom: 0 };
	...
	_pTabComponent._rearrangeBottomButton = function () 
	{
        ...
        var pagerect = { left: 0, top: 0, right: clientWidth, bottom: iMinTop };
		var btnrect = null;
	...
	{		
        ...
        var pagerect = { left: iMaxRight, top: 0, right: clientWidth, bottom: clientHeight };
	...
	_pTabComponent._rearrangeRightButton = function () 
	{
        ...
        var pagerect = { left: 0, top: 0, right: iMinLeft, bottom: clientHeight };
		...
		for (var i = 0; i < tabpagecnt; i++) 
		{
			...
			if (pagecontrol != null)
			{
			    ...
			    {
			        pagecontrol.move(pagerect.left + m_l + b_left,
                                pagerect.top + m_t + b_top,
                                pagerect.right - pagerect.left - m_l - m_r - b_right - b_left,
                                pagerect.bottom - pagerect.top - m_t - m_b - b_bottom - b_top);
			    }
			}
		}
	...
	_pTabComponent._getTabpageClientRectBorder = function (rcPage, border, bordertype, padding, tabpos) 
	{
	    ...
	    if (border)
	    {
	        rcPage.right -= (border._left_width || border._top_width + border_right_width || border._top_width);
	        rcPage.bottom -= (border._bottom_width || border._top_width + border_bottom_width || border._top_width);
	    }
        */
		return rcPage;
	};
	...
	_pTabComponent._getButtonSize = function (index) 
	{
		...
		if (btnmargin)
		{
		    bmleft = btnmargin.left;
		    bmright = btnmargin.right;
		    bmtop = btnmargin.top;
		    bmbottom = btnmargin.bottom;
		}
		*/
	...
	{		
	    if (!this._is_created || this.tabindex == index ||
            ((+index) != (+index)) || index < 0 || (this._child_list && index >= this._child_list.length))
		{
		    return;
	    }
		...
		if (newtabpage) 
		{
		    ...
		    //newtabpage.on_apply_pseudo(newtabpage._pseudo);			
		...
		{
			if (!this.spinupbutton.enable) this.spinupbutton.set_enable(true);
			if (!this.spindownbutton.enable) this.spindownbutton.set_enable(true);
		}
		
	    //click일 경우에는 조정할 필요가 없고, 화면을 벗어난 인덱스로 세팅할 경우에만 재조정이 필요함;
	...
	_pTabComponent._isSpinDownEnable = function () 
	{
	    ...
	    //if (this._lastIndex < this._child_list.length && this._isMultiLine() != true)
	    {
	        return true;
	    }
	    else
	...
	{	    
        // scrollindex부분은 전체적으로 수정해야할듯...(꼬이는 부분이 존재하는듯...)
	...
	{	
		...
		{					
			...
			if (position == "top") 
			{
				//elemstyle.borderTop = "0px " + top.style + " " + topcolor;
				b.set_top_width("0");
			    			
			}
			else if (position == "bottom") 
			{
				//elemstyle.borderBottom = "0px " + bottom.style + " " + bottomcolor;
				b.set_bottom_width("0");			    		
			}
			else if (position == "left") 
			{
				//elemstyle.borderLeft = "0px " + left.style + " " + leftcolor;
				b.set_left_width("0");			    
			}
			else if (position == "right") 
			{
				//elemstyle.borderRight = "0px " + right.style + " " + rightcolor;
				b.set_right_width("0");			    
			}
    ...
    /*
	_pTabComponent._setElementPosition = function (control, position) 	
	{
        var l = 0, t = 0;
        var w, h;
...
if (!nexacro.Tab) 
{
    nexacro.Tab = function (id, position, left, top, width, height, right, bottom, parent)
    {
        ...
        this._tabbutton_obj = null; //하위 버튼 컴포넌트에서 부모로 호출할 때 사용;selectchangetype과 함께 고려;
	...
	_pTab.killTimer = null;//nexacro._emptyFn;
	_pTab.setTimer = null;//nexacro._emptyFn;
	_pTab.loadStyle = null;//nexacro._emptyFn;
	_pTab.setWaitCursor = null;//nexacro._emptyFn;
    ...
    _pTab.on_apply_style_buttonpadding = function (v)
    {
        ...
        for (var i = 0; i < len; i++)
        {           
            this._tabButtons[i].style.set_padding(v._value);
          
            var padding = v;
            var pos = this._tabButtons[i].position;
            pos.right = pos.right + Math.ceil(padding.right);
            pos.bottom = pos.bottom + Math.ceil(padding.bottom);

            this._tabButtons[i]._setElementPosition(this._tabButtons[i].getElement().style, pos);
    ...
    _pTab.on_apply_prop_enable = function (v)
    {
        ...
        if (!enable)
        {
            if (this.spinupbutton) this.spinupbutton.set_enable(enable);
            if (this.spindownbutton) this.spinupbutton.set_enable(enable);
    ...
    _pTab.on_destroy_contents = function ()
    {
        ...
        if (this._tabbutton_obj) this._tabbutton_obj = null;
	...
	_pTab.on_fire_user_onkeydown = function (key_code, alt_key, ctrl_key, shift_key, from_comp, from_refer_comp)
	{
        ...
        /*
		    if ((this._focusobj instanceof nexacro.Tabpage) && key_code == nexacro.Event.KEY_TAB && shift_key)
		    {
		        var oldobj = this._focusobj;
		        var page_first_comp = this._focusobj._getTabOrderFirst();
		        if (this._keydown_filter(null, key_code, alt_key, ctrl_key, shift_key, undefined, from_comp, from_refer_comp))
		        {
		            if (page_first_comp == null && oldobj != this._focusobj)
		            {
		                this._focusobj = oldobj; 
		            }
	...
	{	    
				//if ((this._focusobj instanceof nexacro.Tabpage) != true || ((this._focusobj instanceof nexacro.Tabpage) && key_code == nexacro.Event.KEY_TAB))
	...
	_pTab._on_btn_lbuttondown = function (obj, elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp)
	{
        ...
        this._setTabFocusObj(this._tabButtons[obj._index]); //접근성 동작시 tabbutton때문에 변경
        return ret;
	};    
	...
	_pTab.on_lbuttondown_default_action = function (elem, refer_comp, canvasX, canvasY)
	{
	    ...
	    {	        
            ...
            // extrabutton 눌러 tabpage 삭제 후 extrabutton idx를 못찾음 
            //if (tabbutton_obj.name == "extrabutton")
            //{
	...
	_pTab.insertTabpage = function (strID, nIndex, strURL, strText) 
	{
		...
		if (parseInt(this.tabindex) == _nIndex) 
		{
            ...
            if (oldtab) oldtab.set_visible(false);
		...
		if (this._child_list.length == 1) this.set_visible(true);
	...
	_pTab.removeTabpage = function (nIndex) 
	{
		...
		{			
			...
			if (tabButtons[idx] != undefined) 
			{
			    ...
			    if (tabButtons[idx] == this._last_focused)
			        this._last_focused = null;
			    //if (tabButtons[idx] == this._overedobj)
			...
			if (tabextrabuttons[idx] != undefined)
			{
				//tabbutton_obj.destroy();
				//delete tabbutton_obj;
				this._tabbutton_obj = null;
			}
		}
		...
		*/
		...
		if (child_list.length == 0) this.set_visible(false);
	...
	_pTab._createTabbutton = function (idx) 
	{
		...
		{
		    case "top":
                this._buttonRect[idx] = { left: tabWidth, top: 0, right: tabWidth + rc.right, bottom: rc.bottom };
			    break;
		    case "left":
                this._buttonRect[idx] = { left: tabWidth, top: 0, right: tabWidth + rc.right, bottom: rc.bottom };
			    break;
		...
		var r = rc.right;
		var b = rc.bottom;
        var rctemp = { left: l, top: t, right: r, bottom: b };
        ...
        this._tabButtonBorders[idx] = new nexacro.TabButtonBorderCtrl(tabpage.id + "tabbuttonborder", this.position,
                                        button_rect.left, button_rect.top, button_rect.width, button_rect.height,
                                        null, null, this);
		this._tabButtonBorders[idx]._index = idx;
		
		this._tabButtonBorders[idx].set_visible(false);
		this._tabButtonLeftBorders[idx] = new nexacro.TabButtonBorderCtrl("tabbuttonleftborder", this.position,
                                        button_rect.left, button_rect.top, button_rect.width, button_rect.height,
                                        null, null, this);
...
if (!nexacro.TabButtonCtrl)
{
    ...
    _pTabButtonCtrl.on_change_status = function (status, pseudo)
    {
        ...
        if ((pseudo == "normal" || pseudo == "mouseover" || pseudo == "pushed") &&
            (objTab._tabButtons && objTab._tabButtons[objTab.tabindex] && this.name == objTab._tabButtons[objTab.tabindex].name))
        {
            //return;
            status = "select";
        }
        ...
        switch (status)
        {
            ...
            case "notselect":    
                ...
                if (this._status != "disable") this._status = "enable";
                break;
            case "push":
                this._pushed = true;
                break;
    ...
    {                   	
        ...
        // vml 사용 브라우저는 한쪽 border만 날리는 동작이 안됨.
    ...
    // ====================================================================
    // nexacro.TabButtonCtrl : Event Overriding
    // ====================================================================
    ...
    _pTabButtonCtrl._on_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp)
    {
        ...
        var ret = this.parent._on_btn_lbuttondown(this, elem, button, alt_key, ctrl_key, shift_key, canvasX + this._adjust_left, canvasY, screenX + this._adjust_left, screenY, event_bubbles, fire_comp, refer_comp);	// [FT_45892] ?꾩쓽 濡쒖쭅?????⑥닔濡??듯빀. 12/9/20 cmc

        //if (this.parent._is_canchange)
        //this._on_focus(true);
    ...
    _pTabButtonCtrl._on_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp)
    {
        ...
        return this.parent._on_btn_lbuttonup(this, elem, button, alt_key, ctrl_key, shift_key, canvasX + this._adjust_left, canvasY, screenX + this._adjust_left, screenY, event_bubbles, fire_comp, refer_comp);	// [FT_45892] ?꾩쓽 濡쒖쭅?????⑥닔濡??듯빀. 12/9/20 cmc
    };
    ...
    _pTabButtonCtrl.on_tap_basic_action = function (elem, canvasX, canvasY, screenX, screenY, fire_comp, refer_comp)
    {
        ...
        var ret = nexacro.Component.prototype.on_tap_basic_action.call(this, elem, canvasX, canvasY, screenX, screenY, fire_comp, refer_comp); //이거 호출하지 않아도 되는지 확인 필요;smilekkr;
...
if (!nexacro.Tabpage)
{
    ...
    _pTabPage.bringToFront = null;//nexacro._emptyFn;
    _pTabPage.bringToPrev = null;//nexacro._emptyFn;
    _pTabPage.moveToNext = null;//nexacro._emptyFn;
    _pTabPage.moveToPrev = null;//nexacro._emptyFn;
    _pTabPage.sendToBack = null;//nexacro._emptyFn;
    _pTabPage.sendToNext = null;//nexacro._emptyFn;
    ...
    _pTabPage.set_visible = function (v, skip_focus)
    {
        ...
        //    if (control_elem)
        //    {
        //        var client_elem = control_elem._client_element;
        //        if (client_elem._handle)
        //        {
        //            if (v)
        //            {
        //                client_elem._handle.removeAttribute("hidden");
        //            }
        //            else
        //            {
        //                client_elem._handle.setAttribute("hidden");
        //            }
        //        }
        //    }
        //}
    ...
    _pTabPage._on_bubble_dragenter = function (elem, from_comp, src_comp, src_refer_comp, dragdata, userdata,
    ...
    _pTabPage._on_bubble_dragleave = function (elem, to_comp, src_comp, src_refer_comp, dragdata, userdata,
									button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope)
    {
        if (!this._is_alive) return;
...
if (!nexacro.TabButtonBorderCtrl)
{
    ...
    {
        var parent = this.parent;
        var ret = parent._on_btn_lbuttondown(parent._tabButtons[this._index], parent._tabButtons[this._index]._text_elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp);	// [FT_45892] ?꾩쓽 濡쒖쭅?????⑥닔濡??듯빀. 12/9/20 cmc

        parent._tabButtons[this._index]._on_focus(true);

        //if (parent._is_canchange)
        //   parent._tabButtons[this._index]._on_focus(true);
    ...
    {
        var parent = this.parent;
        return parent._on_btn_lbuttonup(parent._tabButtons[this._index], parent._tabButtons[this._index]._text_elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp);	// [FT_45892] ?꾩쓽 濡쒖쭅?????⑥닔濡??듯빀. 12/9/20 cmc
    };
