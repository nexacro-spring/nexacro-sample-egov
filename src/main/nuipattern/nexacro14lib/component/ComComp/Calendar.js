...
if (!nexacro.Calendar)
{
    ...
    _pCalendarStyle.__get_custom_style_value = function ()
    {
        ...
        if (this.popupsize && this.popupsize._is_empty) val += "popupsize:" + this.popupsize._value + "; ";
        if (this.popupbackground && this.popupbackground._is_empty) val += "popupbackground:" + this.popupbackground._value + "; ";
        if (this.popupborder && this.popupborder._is_empty) val += "popupborder:" + this.popupborder._value + "; ";
        if (this.popupbordertype && this.popupbordertype._is_empty) val += "popupbordertype:" + this.popupbordertype._value + "; ";
        if (this.popupgradation && this.popupgradation._is_empty) val += "popupgradation:" + this.popupgradation._value + "; ";
        if (this.popupalign && this.popupalign._is_empty) val += "popupalign:" + this.popupalign._value + "; ";
        ...
        if (this.popuptype && this.popuptype._is_empty) val += "popuptype:" + this.popuptype._value + "; ";
    ...
    nexacro.Calendar = function (id, position, left, top, width, height, right, bottom, parent)
    {
        ...
        this._event_list = {
            ...
            "oncloseup": 1, "ondayclick": 1, "ondropdown": 1, "oneditclick": 1, "canchange": 1, "cancharchange": 1,
            "onchanged": 1, "onchar": 1, "ongesture": 1, "onmonthchange": 1, "onspin": 1, "ontextchange": 1, "ontextchanged": 1,
    ...
    _pCalendar.on_create_contents = function ()
    {
        ...
        if (control_elem)
        {
            ...
            // only android runtime
            nexacro._create_android_popup_handle(this);
        }
    };
    ...
    _pCalendar.on_created_contents = function ()
    {
        ...
        if (this.calendaredit)
            this.calendaredit.on_apply_custom_setInputRole(this._control_element.accessibility_role);                
    ...
    _pCalendar.on_update_position = function (resize_flag, move_flag)
    {
        ...
        // popupwindow가 열려 있는 상태에서 cell 이동시 popupwindow position 조정
        if (this._isPopupVisible())
        {
            ...
            {
                // Runtime의 경우 SceneGraph Node의 이동이 Async로 이루어지기때문에
                // GetElementPositionInFrame이 잘못된 결과를 가지고 온다.
                // 랜더엔진 내에서 Sync처리를 할 경우 부담이 더 크기때문에 컴포넌트가 관찰하도록 한다.
                var pThis = this;
                ...
                _observer._interval_timer = new nexacro.CallbackTimer(this, function ()
                {
                    ...
                    {
                        // "(Calendar)ElemPos SyncError Detected!!!!"
    ...
    _pCalendar.on_apply_type = function ()
    {
        ...
        if (control_elem)
        {
            ...
            if (this.calendaredit)
                this.calendaredit.on_apply_custom_setInputRole(control_elem.accessibility_role);
        }
    };
    ...
    _pCalendar.on_apply_value = function ()
    {
        ...
        if (control_elem)
        {
            ...
            if (this.calendaredit)
            {
                ...
                {
                    //var status = (this.displaynulltext == "") ? false : true;
                    var status = true;
                }
                else
                {
    ...
    _pCalendar.dropdown = function ()
    {
        ...
        if (this._getPopupType() == "none") return;
    ...
    _pCalendar.updateToDataset = function ()
    {
        ...
        if (this._flag_update2dataset) return true;
    ...
    _pCalendar.on_notify_oneditkillfocus = function (obj, e)
    {
        ...
        // edit -> spinbutton focus 이동시 (클릭에 해당함)
        if (this.type == "spin" && e)
        {
        ...
        //var value = this._makeNormalValue(calendaredit.text);
        var pre_value = this._makeDateObj(this.value);
    ...
    _pCalendar.on_notify_onspinup = function (obj, e)
    {
        ...
        /* spinbutton 동작 후 날짜 문자열 길이가 달라질때 */
        var adjust_caret = 0;
    ...
    _pCalendar.on_notify_mobile_onspinup = function (obj, e)
    {
        ...
        var input_elem = edit._input_element;
        var value = edit.value;
        var caret_pos = this._caret_pos = { begin: edit._edit_base_api._begin_pos, end: edit._edit_base_api._end_pos };
    ...
    _pCalendar._on_killfocus = function (new_focus, new_ref_focus)
    {
        ...
        if (this.type != "monthonly")
        {
            ...
            var text = this._changeYearValue();
            if (this.popupcalendar) this._currentformat = "editformat";
    ...
    _pCalendar._update_popupwindow_position = function ()
    {
        ...
        if (popupwindow)
        {
            ...
            //var scalepos = popupwindow._getScalePosition(popup_control_elem.width, popup_control_elem.height);
    ...
    _pCalendar._setMask = function (v, bApply, value)
    {
        ...
        {
            if ((v == "dateformat" && this.dateformat == "LONGDATE") ||
                (v == "editformat" && this.editformat == "LONGDATE"))
            {
                mask = mask.replace(/\'/g, "\\'");
        ...
        while (idx < _info.format.length)
        {
            ...
            else if (_info._date && _info._date.index == idx)
            {
                ...
                else if (str_len == 4 )
                {
                    ...
                    for (var i = 0; i < week_str_len; i++)
                    {
                        if (week_str.charAt(i) == "-")          // exception case
    ...
    {
        var year = "", month = "", day = "", hour = "", min = "", sec = "", millisec = ""; // Associated RP_38767)
        var mask_value = "";
        var _info = null;
        var is_rtl_month = false;
        var is_rtl_date = false;
        ...
        month = v.substr(4, 2);
        day = v.substr(6, 2);
        hour = v.length >= 9 ? v.substr(8, 2) : (v == "") ? "  " : "00";
        min = v.length >= 11 ? v.substr(10, 2) : (v == "") ? "  " : "00";
        sec = v.length >= 13 ? v.substr(12, 2) : (v == "") ? "  " : "00";
        millisec = v.length >= 15 ? v.substr(14, 3) : (v == "") ? "   " : "000";
        */

        // Associated RP_38767)dateformat은 다양한 형태가 있을 수 있으므로, 기존처럼 dateformat과 상관없이 무조건 substr을 하면 안된다.
        if (!nexacro._isNull(format_info) && nexacro._isNull(format_info._year) && nexacro._isNull(format_info._mon))
        {   // 년도와 월의 format이 없는경우(dd)
            day = v.substr(0, 2);
    ...
    _pCalendar._makeNormalValue = function (edit_value)
    {
      ...
      //  var year = _info._year ? edit_value.substr(_info._year.index, _info._year.length).padLeft(4, "0") : "0000";
    ...
    _pCalendar._makeFormatInfo = function (format)
    {
        ...
        for (var i = 0; i < format_len; i++)
        {
            char_info = { ch: format_real.charAt(i), idx: i };
        ...
        if (year_format)
        {
            year = { mask: year_format[0], index: year_format.index, length: year_format[0].length };
            list.push(year);
        }
        if (mon_format)
        {
            mon = { mask: mon_format[0], index: mon_format.index, length: mon_format[0].length, single_digit:true };
            list.push(mon);
        }
        if (day_format)
        {
            day = { mask: day_format[0], index: day_format.index, length: day_format[0].length, single_digit: true };
            list.push(day);
        }
        if (date_format)
        {
            date = { mask: date_format[0], index: date_format.index, length: date_format[0].length };
        }
        if (hour_format)
        {
            hour = { mask: hour_format[0], index: hour_format.index, length: hour_format[0].length };
            list.push(hour);
        }
        if (min_format)
        {
            min = { mask: min_format[0], index: min_format.index, length: min_format[0].length };
            list.push(min);
        }
        if (sec_format)
        {
            sec = { mask: sec_format[0], index: sec_format.index, length: sec_format[0].length };
            list.push(sec);
        }
        if (millisec_format)
        {
            millisec = { mask: millisec_format[0], index: millisec_format.index, length: millisec_format[0].length };
            list.push(millisec);
        }
        ...
        return { format: format_real, format_info: format_info, format_list: list, _year: year, _mon: mon, _day: day, _date: date, _hour: hour, _min: min, _sec: sec, _millisec: millisec };
    ...
    _pCalendar._getPopupSizeArr = function ()
    {
        ...
        {
            return { width: parseInt(arr[0], 10), height: parseInt(arr[0], 10) };
        }
    };
    ...
    //_pCalendar._getLocale = function ()
    //{
    //    var locale = this.locale;
    //    if (locale)
    //    {
    ...
    //    else
    //    {
    //        return this._getLocaleFromBrowserLang();
    //    }
    //};

    //_pCalendar._getLocaleFromBrowserLang = function ()
    //{
    //    if (nexacro.BrowserLang == "ko" || nexacro.BrowserLang == "ko-KR" || nexacro.BrowserLang == "ko-kr")
    //    {
    //        return "ko_KR";
    //    }
    //    else if (nexacro.BrowserLang == "ja" || nexacro.BrowserLang == "ja-JP" || nexacro.BrowserLang == "ja-jp")
    //    {
    //        return "ja_JP";
    //    }
    //    else if (nexacro.BrowserLang == "en" || nexacro.BrowserLang == "en-US" || nexacro.BrowserLang == "en-us")
    //    {
    //        return "en_US";
    //    }
    //    else
    ...
    _pCalendar._getSplitDate = function (v)
    {
        ...
        return { year: year, mon: mon, day: day, hour: hour, min: min, sec: sec, millisec: millisec };
    };
    ...
    _pCalendar._setPreValueAndText = function (pre_value)
    {
        ...
        //this._pretext = this._makeCalendarText(this._makeMaskValue_normal(pre_str));
    ...
    _pCalendar._setPostValueAndText = function (post_value)
    {
        ...
        //this._posttext = this._makeCalendarText(this._makeMaskValue_normal(post_str));
    ...
    //===============================================================
    // nexacro.CalendarButtonCtrl : Button
    //===============================================================
    ...
    {
        //on touch devices, application.enabletouchevent == false
    ...
    //===============================================================
    // nexacro.CalendarButtonCtrl : Style Part
    //===============================================================
    ...
    //===============================================================
    // nexacro.CalendarEditCtrl : Style Part
    //===============================================================
    ...
    _pCalendarEditCtrl.on_apply_custom_setInputRole = function (v)
    {
        var input_elem = this._input_element;
        if (input_elem)
        {
            this._control_element.accessibility_role = v;
    ...
    //===============================================================
    // nexacro.CalendarPopupWindow : Popup
    //===============================================================
