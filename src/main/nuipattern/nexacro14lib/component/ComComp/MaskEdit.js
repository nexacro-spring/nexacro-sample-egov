...
if (!nexacro.MaskEdit)
{
    ...
    nexacro.MaskEdit = function (id, position, left, top, width, height, right, bottom, parent)
    {
        ...
        this._event_list = {
            ...
            "oneditclick": 1,
            "canchange": 1, "cancharchange": 1,
            "onchanged": 1, "onchar": 1, "ontextchange": 1, "ontextchanged": 1,
            "oncontextmenu": 1,
            "onfling": 1, "onflingstart": 1, "onflingend": 1,
            "onpinch": 1, "onpinchstart": 1, "onpinchend": 1,
            "onslide": 1, "onslidestart": 1, "onslideend": 1,
            "ontouchstart": 1, "ontouchmove": 1, "ontouchend": 1,"onlongpress": 1
        };
    ...
    _pMaskEdit.on_created_contents = function (doc)
    {
        ...
        if (input_elem)
        {
            ...
            input_elem._is_maskedit = true; //only runtime;
    ...
    _pMaskEdit.on_fire_ontextchange = function (event_info)
    {
        ...
        if (this.ontextchange && this.ontextchange._has_handlers)
        {
            ...
            if (ret && event_info.chartext != evt.chartext)
            {
                ...
                if (this.type == "number")
                {
                    var filterChar = api._is_maskchar(evt.chartext);

                    if (api.isSign(filterChar))
                    {
                        api._apply_sign_filter(filterChar);
                    }
                    else if (api.isComma(filterChar))
                    {
                        api._apply_comma_filter(filterChar);
                    }
                    else if (api.isDigit(filterChar))
                    {
                        api._apply_digit_filter(filterChar);
                    }
