...
// nexacro.Map 
// ==============================================================================
if (nexacro.Device && nexacro.OS == "Android") //Android 
{ 
		if (!nexacro.Map) 
		{      	
		    ...
		    _pMap.on_create_contents = function() 
			{
				...
				if (control_elem) 
				{
					...
					//this._ifrm_elem = new nexacro.WindowedWebBrowserElement(control_elem);
			...
			_pMap.set_viewmode = function(v) 
		    {
		  	    ...
		  	    {
		  	    	/*
		  	        if (v == 0 || (typeof (v) == "string" && v == "0"))
		  	        {
		  	            this.viewmode = 1;
		  	        }
		  	        else if (v == 1 || (typeof (v) == "string" && v == "1"))
		  	        {
		  	            this.viewmode = 2;
		  	        }
		  	        else if (v == 2 || (typeof (v) == "string" && v == "2"))
		  	        {
		  	            this.viewmode = 4;
		  	        }
		  	        else if (v == 3 || (typeof (v) == "string" && v == "3"))
		  	        {
		  	            this.viewmode = 3;
		  	        }
		  	        else
		    ...
		    {	
			    ...
			    {
					// set location	nLatitude, nLongitude,
					var strlength = 0;
					try 
					{
			...
			*/
			...
			_pMap.removeItem = function(strItemID)
			{
		        ...
		        if (typeof (this.items[strItemID]) == "undefined")
		        {
		            this.on_fire_onerror(this, "onerror", "1501", "item is not exist.");
		            return false;
				}
		        else
		        {
...
// ios    
else if (nexacro.Device || nexacro.Browser != "Runtime" && !(nexacro.Browser == "IE" && nexacro.BrowserVersion <= 8)) // iOS && HTML5
{
		if (!nexacro.Map)
		{
            //호출 시점 변경
            ...
            */
			...
			_pMap.on_apply_text = function()
			{
				...
				if (this._control_element)
				{
					...
					if (!text_elem)
					{
						...
						text_elem.setElementSize(this._client_width,
								this._client_height);
			...
			_pMap.on_create_contents = function()
			{
				...
				if (control_elem)
				{
					...
					var map_elem = this._map_elem = new nexacro.Element(
							control_elem);
					map_elem.setElementSize(this._client_width,
							this._client_height);
			...
			_pMap.load = function(bUseSensor, nLatitude, nLongitude,
					constViewMode, nZoomLevel)
			{
				...
				for (var i = 0; i < this.itemsname.length; i++)
				{
					...
					{
						_pMap.addMarker(this.itemsname[i],
								this.items[this.itemsname[i]]);
					}
					else if (this.items[this.itemsname[i]]._type == 2)
					{
						_pMap.addPolyline(this.itemsname[i],
								this.items[this.itemsname[i]]);
					}
					else if (this.items[this.itemsname[i]]._type == 3)
					{
						_pMap.addPolygon(this.itemsname[i],
								this.items[this.itemsname[i]]);
					}
				}
				...
				google.maps.event.addListener(google_map, 'click', function(e)
				{
				   if (parentMap.onclick && parentMap.onclick._has_handlers && parentMap.enableevent)
				    ...
				    //RP 44241 iOS에서 MapMarker의 풍선말을 닫는 기능 필요(X선택 불가. Map click시 infowindow.close()
				...
				{
				    //console.log("zoom_changed" + google_map.getZoom());
	        ...
	        {
	     	   	  //console.log("marker onmapdragend!");  	  
			...
			_pMap.addPolygon = function(polygonName, polygonObj)
			{
				...
				google.maps.event.addListener(_items[polygonName], 'click',
						...
						{
							// overlayclick call..
							_pMap.gmap_onoverlayclick(
									polygonObj/* _items[polygonName] */,
			...
			_pMap.addPolyline = function(polylineName, polylineObj)
			{
				...
				google.maps.event.addListener(_items[polylineName], 'click',
						function(e)
						{
							// overlayclick call..
							_pMap.gmap_onoverlayclick(
									polylineObj/* _items[polylineName] */,
									e.latLng.lat(), e.latLng.lng(), 0, 0);
							// HJSHIN_20121009
						});
			};
			...
			{
				this.on_fire_ondragstart(this, new nexacro.MapDragEventInfo(
						"ondrag"));
			}

			_pMap.gmap_ondragenter = function(objData)
			{
				this._fire_ondragenter(this, new nexacro.MapDragEventInfo(
						"ondragenter"));
			}
			...
			{
				this._fire_ondragleave(this, new nexacro.MapDragEventInfo(
						"ondragleave"));
			}
			...
			{
				//console.log("_on_click override");
			}
			...
			_pMap._fire_onclick = function(obj, e)
			{
				...
				{
					this.bClicked = false;
					var evt = new nexacro.MapClickEventInfo(this, obj,
							"onclick", e, this._location, this._pixel);
					return this.onclick._fireEvent(this, evt);
				}
				else
				{
			...
			_pMap._fire_onoverlayclick = function(obj, e)
			{
				...
				{
					var evt = new nexacro.MapClickEventInfo(this,
							this.overlayfromobj, "onoverlayclick", e,
							this._location, this._pixel);
			...
			_pMap.on_fire_onerror = function(obj, e)
			{
				if (this.onerror && this.onerror._has_handlers) 
		        {
					var evt = new nexacro.MapErrorEventInfo(this, "onerror",
			...
			nexacro.MapMarker = function(name, obj)
			{
				...
				this.location = {
					...
					set_latitude : function(lat)
					{
						...
						{
							__parent._map.setMarkerOptions(__parent._name,
									__parent);
						}
						return true;
					},
				...
				this.style = {
					...
					strokepen : {
						...
						set_width : function(v)
						{
							...
							if (__parent._map != null)
							{
								__parent._map.setPolylineOptions(
										__parent._name, __parent);
							}
							return true;
						},
					...
					set_strokepen_width : function(v)
					{
						...
						if (__parent._map != null)
						{
							__parent._map.setPolylineOptions(__parent._name,
									__parent);
						}
				...
				this.style = {
					...
					strokepen : {
						...
						set_width : function(v)
						{
							...
							if (__parent._map != null)
							{
								__parent._map.setPolygonOptions(__parent._name,
										__parent);
							}
							return true;
						},
