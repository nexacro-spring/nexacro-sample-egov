...
if (!nexacro.PopupMenuItem)
{
    ...
    _pPopupMenuItem.on_change_containerRect = function (width, height)
    {
        ...
        var text_height = 0, parent_height = 0; // text height 로 대체 뭘하는 거지? 높이는  itemheight로 결정 하는것
    ...
    _pPopupMenuItem.on_apply_mouseover = function (isovered)
    {
        ...
        if (isovered)
        {            
            this._stat_change("normal", "mouseover"); // 2014-06-23 pss [REQ_36478] - status normal로 동작
        }
        else
        {
...
if (!nexacro.PopupMenu_Style)
{
    ...
    // ==============================================================================
    // nexacro.PopupMenu_CurrentStyle
    // ==============================================================================
...
if (!nexacro.PopupMenu)
{
    ...
    nexacro.PopupMenu = function (id, position, left, top, width, height, right, bottom, parent)
    {
        ...
        this._want_tab = true;
        this._selected_itemindex = -1;  // for MenuCloseEventInfo.isselect (which mouse click or enter key)
    ...
    _pPopupMenu.on_find_CurrentStyle_itemalign = function (pseudo)
    {
        ...
        return this._find_pseudo_obj("itemalign", pseudo, "align"); //this._find_pseudo_obj("align", pseudo, "align");
    };
    ...
    _pPopupMenu.on_apply_style_expandimage = function (expandimage)
    {
        ...
        var img_url = this._load_image(expandimage, "exp");  // 1 is exp_img flg
    ...
    _pPopupMenu.on_apply_prop_rtldirection = function ()
    {
    	...
    	//var rootComp = this._getRootComponent(this);
    	//var _rtldirection = rootComp._rtldirection;
    	var _rtldirection = this._rtldirection;
    ...
    _pPopupMenu.on_create_contents = function ()
    {
        ...
        if (!this._is_subcontrol)
        {
            ...
            {
                this._load_image(curstyle.checkboximage, "chk"); // 0 is check_img flg
            }

            if (curstyle.expandimage)
            {
                this._load_image(curstyle.expandimage, "exp"); // 1 is exp_img flg
            }
        }
    ...
    _pPopupMenu.on_fire_user_ondrag = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, refer_comp, self_refer_comp)
    {
        if (this.ondrag && this.ondrag._has_handlers)
        {
            ...
            var evt = new nexacro.MenuDragEventInfo(this, "ondrag", refer_comp.id, dragData, null, this, self_refer_comp, from_comp, refer_comp,
                                            button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, self_refer_comp.parent.level, self_refer.index, self_refer._bindindex);
                                    ...
                                    button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp)
    {
        ...
        {
            var refer_comp = from_refer_comp._overedobj || from_refer_comp;
            var evt = new nexacro.MenuDragEventInfo(this, "ondrop", refer_comp.id, dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp,
                                            button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp.parent.level, refer_comp.index, refer_comp._bindindex);
                                        ...
                                        button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp)
    {
        ...
        {
            var refer_comp = from_refer_comp._overedobj || from_refer_comp;
            var evt = new nexacro.MenuDragEventInfo(this, "ondragenter", refer_comp.id, dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp,
                                            button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp.parent.level, refer_comp.index, refer_comp._bindindex);
                                        ...
                                        button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp)
    {
        ...
        {
            var iteminfo = this._last_mouseleave_iteminfo;
            var evt = new nexacro.MenuDragEventInfo(this, "ondragleave", refer_comp.id, dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp,
                                        ...
                                        button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp)
    {
        ...
        {
            var refer_comp = from_refer_comp._overedobj || from_refer_comp;
            var evt = new nexacro.MenuDragEventInfo(this, "ondragmove", refer_comp.id, dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp,
                                            button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp.parent.level, refer_comp.index, refer_comp._bindindex);
        ...
        if (this._is_subcontrol == false)
        {
            ...
            {       // none Accessibility
                switch (keycode)
                {
                    ...
                    case E.KEY_LEFT:
                        //if ((pThis.parent instanceof nexacro.Form) && pThis.level == 0)
                    ...
                    case E.KEY_ENTER:
                        ...
                        else
                        {
                            ...
                            if (!popupexpand)
                            {
                                //this._item_killfocus(rootComp._menuitemonmouseenter, true);
                            }
                            rThis.on_notify_menuitem_onlbuttondown(rootComp._menuitemonmouseenter);
...
/*
    ...
    _pPopupMenu._load_image = function (image, strflag)  // flag : 0 -> checkimg , 1 -> expimg , 2-> iconimg
    {
        var control_elem = this._control_element;
        if (control_elem)
        {
    ...
    _pPopupMenu._reCalcSize = function ()
    {
        ...
        if (ds && this.captioncolumn)
        {
            ...
            if (_expandimage)
            {
                ...
                if (expimgwidth == undefined) { expimgwidth = 0; } 
                if (expimgheight == undefined) { expimgheight = 0; } 
            }
            ...
            var _default_gap = 20; // 어떤 정보에 의해서 인지 조사필요
            ...
            if (!childframe) childframe = frame;

            var bodyHeight = childframe._adjust_height;                        
            ...
            while (comp)
            {
                ...
                {
                    menu_bottom = comp._adjust_top + comp._adjust_height;
                    break;
                }
            ...
            if (_buttonRect[0])
            {
                ...
                {
                    if (this._scrollIndex == 0) _item_top = this._spin_height;
                    else _item_top = -this._spin_height * this._scrollIndex;
                }
            }
            ...
            for (var i = 0; i < this._lineItems.length; i++)
            {
                ...
                else
                {
                    ...
                    var rect1 = { textWidth:0, hotkeyWidth:0, itemHeight:0,  defaultGap:0};
                    ...
                    //item._updateElementPositions(textwidth, hotkeywidth, item_h, _default_gap);
    ...
    _pPopupMenu._calcSpinButton = function ()
    {
        ...
        //if (_buttonRect[len - 1].bottom > this._adjust_height-this._spin_height || this._is_spin_visible)
        ...
        {            
            ...
            if (this._is_spin_visible) top = 0;
    ...
    _pPopupMenu._updateMenuItemPosition = function ()
    {
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            var buttonRect = _buttonRect[i];
            item.move(buttonRect.left, buttonRect.top, buttonRect.width, buttonRect.height);
            ...
            lineitem._updateElementPositions(buttonRect_elem.textWidth, buttonRect_elem.hotkeyWidth,
                                                buttonRect_elem.itemHeight, buttonRect_elem.defaultGap);
        }
    };
    ...
    // ==============================================================================
    // PopupMenuCtrl : PopupMenu used as Control ==> use Event Notifiers, style
    // control - controlType
    // ==============================================================================
