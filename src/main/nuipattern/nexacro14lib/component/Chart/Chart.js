...
if (!nexacro.Chart)
{
    ...
    _pChart._recalcXAxisItemInfos = function(rc) 
    {
        ...
        // calc axis item cnount & item_div 
    ...
    _pChart._recalcSeriesInfos = function() 
    {
        ...
        if (barcnt > 0) 
        {
            ...
            // adjust barspacing && baritemspacing
    ...
    {
        if (this.enableredraw == false) return;
    ...
    _pChartTitle.ondraw = function(canvas) 
    {
        ...
        {
//            var offset_x = this._client_rect.getHeight();
   ...
   //==============================================================================
    ...
    _pChartLegend._recalcLayout = function() 
    {
        ...
        if (seriesset && seriesset.length > 0) 
        {
            ...
            for (var i = 0; i < len; i++) 
            {
                ...
                if (item.item_width > wmax) wmax = item.item_width;
                wsum += item.item_width;
                if (item.item_height > hmax) hmax = item.item_height;
    ...
    _pChartAxis.on_updateDecimalless = function() 
    {
       ...
       ret = ret.sort(function(a, b) {return b-a})[0];
    ...
    _pChartAxis._recalcLayout = function(rc) 
    {
        ...
        else 
        {
            ...
            if (unit_val == undefined) unit_val = 100;
            ...
            if (org_val != undefined) 
            {
                ...
                if (org_size < main_size) org_size = main_size;
            ...
            if (mark_val != undefined) 
            {
                ...
                if (mark_size < main_size) mark_size = main_size;
            ...
            if (min_val == undefined || max_val == undefined) 
            {
                ...
                if (dataset) 
                {
                    ...
                    {
                        if (chk_min) min_val = parent.yaxis._min_val;
                        if (chk_max) max_val = parent.yaxis._max_val;
                    }
                }
            ...
            if (cnt == 0) { unit_max++; cnt++; }
            
            // set united min_val, max_val;
    ...
    _pChartAxis._drawXAsis = function(canvas) 
    {
        ...
        var base_pos = data_rc.left - this._adjust_left;//this.position.left;
        var base_width = data_rc.right - data_rc.left;
    ...
    _pChartAxis._drawYAsis = function(canvas) 
    {
        ...
        var parent = this.parent;
        var data_rc = parent._data_area;
        var base_pos = data_rc.top - this._adjust_top;//this.position.top;
        var base_width = canvas.width;
        var base_height = data_rc.bottom - data_rc.top;

        var isY2 = false;
        if (this.id == "y2axis") isY2 = true;
        ...
        var center = axis_size / 2;
        var offset = 0 ; 
        if (main_line)   offset += main_line._width / 2;
        ...
        if (bdraw_main) 
        {
            ...
            if (axis_size) 
            {
                ...
                if(main_line)   pos += main_line._width;
        ...
        if (org_val <= max_val && org_val >= min_val && org_val != mark_val) 
        {
            ...
            if (org_color) org_color = org_color._value;
            else org_color = "black";
            ...
            if (text_color != "") 
            {
                ...
                switch (text_align && text_align.halign)
                {
                    case "left":
                        ...
                        text_pos = "left";
                        if (text_padding)    text_offset = text_padding.right
                        break;
                    case "right":
                        guide_offset_x = guide_size;
                        text_pos = "right";
                        if (text_padding)   text_offset = text_padding.left;
                        break;
                }
                switch (text_align && text_align.valign)
                {
                    case "top":
                        ...
                        {
                            text_pos = "top";
                            if (text_padding)   text_offset = text_padding.bottom;
                        }
                    ...
                    case "bottom":
                        ...
                        {
                            text_pos = "bottom";
                            if (text_padding)   text_offset = text_padding.top;
                        }
                        else if (text_pos == "left") 
                        {
                            text_pos = "leftbottom";
            ...
            if (chk_mark) 
            {
                ...
                if (chk_mark) 
                {
                    for (var i = 0; i < item_cnt; i++) 
                    {
                        ...
                        if (val != undefined) 
                        {
                            parent._drawMark(canvas, _x[i] - gap, _y[i] - gap, 
                                    markshape, marksize, markfillcolor, markborderpen);
                        }
                    }
                }
            }
