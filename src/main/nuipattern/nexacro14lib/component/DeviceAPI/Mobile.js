...
if (!nexacro.Sms)
{
	...
	{	// SMS 보내기
		...
		{
            // fix return
			//this._onerror({phonenumber:this.phonenumber,message:this.message,errorcode:'00001',errormsg:'parameter error'});
			return false;
		}
	};
	...
	{		// 모바일 기기의 Sms에서 메시지 수신 모니터링을 종료한다.
	    if (nexacro.Device.curDevice == 0)
	    {
			// 모바일 기기의 Sms에서 메시지 수신 모니터링을 종료한다.
	        if (nexacro.__getMobileProductType().toLowerCase() == "shw-m380s")
	...
	_pSms.readMessageList = function()
	{
	    ...
	    {
			if (nexacro.__getMobileProductType().toLowerCase() == "shw-m380s" ||
				nexacro.__getMobileProductType().toLowerCase() == "lg-su760")
			{
				// Android 미지원 기종.
				this.sendUnsupportedFunc();
				return;
	...
	_pSms.deleteMessage = function(_id)
	{
	    ...
	    {
			if (nexacro.__getMobileProductType().toLowerCase() == "shw-m380s" ||
				nexacro.__getMobileProductType().toLowerCase() == "shw-m250s" ||
				nexacro.__getMobileProductType().toLowerCase() == "lg-su760")
			{
				// Android 미지원 기종.
				this.sendUnsupportedFunc();
...
//==============================================================================
// nexacro.SmsMessagListEventInfo
...
if (!nexacro.AudioPlayer)
{
	...
	{
	    var pcheck = this.paramck_AudioLoad(v); // 파라미터 체크
	    if (pcheck == true)
	    {
	        this.url = v;
	        return true;
    ...
    {
        //모바일 기기에서 재생할 오디오 파일을 URL로부터 로딩한다.
		// 이부분에서 Native 메소드를 호출하는 부분 구현 필요
        this.startpos = -1;
        this.stoppos = -1;

        var pcheck = this.paramck_AudioLoad(strUrl); // 파라미터 체크
		if (pcheck == true)
		{
			this.url = strUrl;
			if (nexacro.Device.curDevice == 0)
    ...
    _pAudioPlayer.movePos = function(nPos)
    {
        ...
        {
            // fix return
//			this._onerror({errorcode : '00001', errormsg : 'parameter error'});
            return false;
		}
    };
	...
	*/
	...
	_pGeolocation.getCurrentPosition = function(nAccuracy, nTimeout)
	{
	    ...
	    else
	    {
            // fix return
            return false;
//        	this._onrecverror({errorcode : '00001', errormsg : 'parameter error'});
        }
	};	
	...
	_pGeolocation.watchStart = function(nAccuracy, nIntervalTime)
	{
	    ...
	    {
	        // getCurrentPostion과 param 체크 방법이 동일하다.
...
if (!nexacro.Acceleration)
{
	...
	_pAcceleration.getCurrentAcceleration = function(nAccuracy)
	{
	    ...
	    var pcheck = this.parmack_AccelgetCurrentPosition(this.accuracy); // 파라메터 체크
		if (pcheck == true)
		{
		    if (nexacro.Device.curDevice == 0)
		    {
...
//			this._onrecverror({errorcode : '00001', errormsg : 'parameter error'});
	...
	_pAcceleration.watchStart = function(nAccuracy, nIntervalTime)
	{
	    ...
	    var pcheck = this.pramck_AccelwatchStart(nIntervalTime, nAccuracy); // 파라메터 체크
		if (pcheck == true)
		{
			//this.nIntervalTime = nIntervalTime || "200";
            //this.accuracy = nAccuracy || "0";
		    if (nexacro.Device.curDevice == 0)
		    {
...
if (!nexacro.Vibrator) {
	...
	_pVibrator.play = function()
	{
	    ...
	    else
	    {
	        ...
	        if (nexacro.Device.isphone != 1) //isPad
	        {
...
if (!nexacro.FileDialog) 
{
	nexacro.FileDialog = function(name, obj) 
	{
		...
		this.EnvironmentPath = 1;     // 외장 2, 내장 1 , 
		this.async = true;
	...
	{	
		...
		var params = '{"strTitle":"'+strTitle
						...
						+'","EnvironmentPath":"'+ this.EnvironmentPath +'"}';//EnvironmentPath 파라미터 (안드로이드 전용)
        var jsonstr;
...
if (nexacro.OS == "iOS" && !nexacro.VirtualFile)
{
    nexacro.VirtualFile = function (name, obj)
    {
        ...
        this._event_list = {
           "onsuccess":1, "onerror":1, "getEvent":1
        };	
    ...
    nexacro.VirtualFile.EnvironmentPath = 1; // 외장2 , 내장 1 ,  (안드로이드 전용) 기본값 1
    ...
    _pVirtualFile.open = function (strFileName, nOption)
	{
		...
		else if (strFileName != null)
		{
			...
			this.fullpath =  strFileName; // this.fullpath  설정
		}
		else
		{
			return false;
		...
		this.strFilename=""; //네이트브에 넘기는 Alias 를 제외한 경로+파일명
    ...
    _pVirtualFile.seek = function (nOffset, nOption)
	{
		...
		if (nOption == null)
		{
		 	...
		 	this.nOption = VirtualFile.seekCurrent; //디폴트 파일의  현재 위치
		}
    ...
    // 2013.11.18 v13변경 스펙에 deprecated예정.
    _pVirtualFile.remove = function (strFilePath)
	{
		this._strFilePath ="";//필수
	...
	{
		this.strPath = strPath;             //필수
		this.strSearchExpr = strSearchExpr; //필수
		this.constOption= constOption;      //옵션
    ...
    _pVirtualFile._onsuccess = function (objData)
	{
		...
		if (temptxtlen > 0)
		{
			//console.info("vfile : " + typeof(objData.textdata));
		...
		//alert("objData.fileisexist : " + objData.fileisexist);
		//alert("objData.reason : " + objData.reason);
    ...
    _pVirtualFile.paramck_folderName = function (strName)
	{
	    ...
	    /*
        if (strName.indexOf("\"") != -1) { return false; }
        if (strName.indexOf("/") != -1) { return false; }
        if (strName.indexOf(":") != -1) { return false; }
        if (strName.indexOf("*") != -1) { return false; }
        if (strName.indexOf("?") != -1) { return false; }
...
if (!nexacro.Camera)
{
    nexacro.Camera = function(name, obj)
    {
        ...
        this.encodingtype = "JPEG";				 					// 이미지 인코딩 타입
        this.gettype = "0";                                      // 리턴받을 이미지 형태 (‘url’, ‘data’)
        this.enableevent = true;
    ...
    _pCamera.takePicture = function()
    {
        ...
        if (this.paramck_takePicture(this.imagequality, this.imagewidth, this.imageheight, this.encodingtype, this.gettype))
        {
            ...
            params += '", "gettype":"' + this.gettype.toString();	// 0 : Url, 1: Data
            params += '"}';
        ...
        {
            // fix return
            //	    	this._onerror({errorcode : '00001', errormsg : 'parameter error'}); //파라미터 개수가 1 또는 2가 아니면 ..error
            //			return false;
        }
        //        return true;
    };
...
if (!nexacro.ImagePicker)
{
    ...
    {
        // 이미지를 얻기위해 카메라 어플리케이션을 구동시킨다.		
        /*this.GetType = strGetType;   //(0:imageurl, 1:imagedata)

	    if (typeof(strGetImageType) == "undefined"){
	    	strGetImageType = 1;
	    }
	    strGetImageType += "";
	    
	    if (!(strGetImageType == "0" || strGetImageType == "1")){
	    	return false;
	    }
		this.GetImageType = strGetImageType; //(0: JPEG, 1:PNG)*/

        if ((strGetType != null && typeof (strGetType) != "undefined") &&
			 ((strGetType == 0) || (strGetType == 1) || (strGetType == "0") || (strGetType == "1")))
            this.gettype = strGetType;

        if ((strGetImageType != null && typeof (strGetImageType) != "undefined") &&
			 ((strGetImageType == 0) || (strGetImageType == 1) || (strGetImageType == "0") || (strGetImageType == "1")))
    ...
    //===============================================================
    /*_pImagePicker.paramck_ImagePicker_asyncOpen = function(strGetType, strGetImageType) {
    if (strGetType == null || typeof(strGetType) == "undefined" ) {
        return false;
    } else if (!((strGetType == 0) || (strGetType == 1) || (strGetType == "0") || (strGetType == "1"))) {
        return false;
    }
    
    if (strGetImageType == null || typeof(strGetImageType) == "undefined" ) {
        return false;
    } else if (!((strGetImageType == 0) || (strGetImageType == 1) || (strGetImageType == "0") || (strGetImageType == "1"))){
        return false;
    }
    
    if (!nexacro.Device.publicNumCheck(strGetType,strGetImageType)){
        return false;
    }
    return true;
    }*/
...
if (!nexacro.AudioRecorder)
{
	...
	_pAudioRecorder._onrecord = function(objData) {
		...
		this.bpaused = false;
		// rec status manage
//		var e = new nexacro.AudioEventInfo("onrecord", objData.reason, objData.url, objData.playtime, objData.curtime, null, null, null);
//		this.$fire_onrecord(this, e);
	};
...
if (!nexacro.Contact)
{
    nexacro.Contact = function(name, strUniqueid, strDisplayname, strFamilyname, strGivenname,
						...
						arrOrganizations, arrPhotos, obj)
    {
        this.enableevent = true;
    ...
    {
        //        if (nexacro.Device.pramck_contactString(v) == true) {
        //            this.contactname.displayname = v;
        //            return true;
        //        }
        //        return false;
    };
...
if (!nexacro.ExternalAPI)
{
	...
	{
	    if (typeof (strApplicationID) == "undefined" || strApplicationID.length == 0)
	    {
    	 	return false;
	    }
	    else
    ...
    _pLiteDBConnection.open = function (strDataSource, constOpenFlag)
    {
        ...
        if (this.paramck_asyncOpen(this.datasource, this.openflag))
        {
            ...
            if (nexacro.Device.curDevice == 0)
            {
	        	...
	        	{
	        		var _filecache = application._getFileCache(this.datasource);
	        		if (null != _filecache)
	        		{
	            		EnvironmentPath = "3";
						var params = '{"datasource":"'+_filecache+'", "openflag":"'+this.openflag + '", "busytimeout":"'+this.busytimeout + '", "EnvironmentPath":"'+ EnvironmentPath+'"}';
						var jsonstr = '{"id":'+this._id+', "div":"SQLConnection", "method":"asyncOpen", "params":'+params+'}'; 
						nexacro.Device.exec(jsonstr);
						return true;
	            	}
	            	else
	         	...
	         	var params = '{"datasource":"'+this.datasource.substring(9, this.datasource.length)+'", "openflag":"'+this.openflag + '", "busytimeout":"'+this.busytimeout + '", "EnvironmentPath":"'+ EnvironmentPath+'"}';
	            var jsonstr = '{"id":'+this._id+', "div":"SQLConnection", "method":"asyncOpen", "params":'+params+'}'; 
	            nexacro.Device.exec(jsonstr);
            }
            else
            */
            {
                ...
                else
                {
	            	...
	            	if (null != _filecache)
	            	{
	            		var params = '{"datasource":"'+_filecache+'", "openflag":"'+this.openflag + '", "busytimeout":"'+this.busytimeout + '"}';
						var jsonstr = '{"id":'+this._id+', "div":"SQLConnection", "method":"openDB", "params":'+params+'}';  
						nexacro.Device.exec(jsonstr);
	            	}
	            	else
...
{
    nexacro.XPushEventInfo = function (eventid, reason, serverip, serverport, command, pushmessageobject)
    {
...
{
    nexacro.XPushErrorEventInfo = function (strEventId, intErrorCode, strErrorMsg, strServerip, strServerport, strCommand, pushmessageobject)
    {
        this.eventid = strEventId;												// 이벤트ID
        this.errortype = "ObjectError";
		this.statuscode = intErrorCode;
...
//==============================================================================
//nexacro.XPushRequestMessageCountEventInfo
//XPush
//==============================================================================
...
if (nexacro.OS == "iOS" && !nexacro.XPush)
{
    nexacro.XPush = function (name, obj)
    {
        ...
        this.connectip = "";
        this.connectport = "-1";
		this.connectSuccess = false;  //connect 성공 여부 (only iOS)
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
							if (strCommand == "ADDF")
							{
								jsonstr = '{"id":' + this._id + ', "div":"XPush", "method":"ADDF", "params":' + JSON.stringify(cc) + '}';
								...
								if (!duplicated) this.commandlist.push(cc);
							}
							else if (strCommand == "DELF")
							{
								...
								if (duplicated) this.commandlist.splice(i,1);
							}
                        }
                    }
                }
    ...
    _pXPush.requestMessage = function ()
    {
		if (this.connectSuccess)
		{
			...
			if (messagekeys.length > 0)
			{
				...
				//	var  jsonstr = '{"id":' + this._id + ', "div":"XPush", "method":"requestMessage", "params":' + JSON.stringify(cc) + '}';
           ...
           for (var j = 0; j < this.commandlist.length; j++)
           {
               ...
               if (cc.messagetype == recv.type)
               {
                   ...
                   for (var dlidx = 0; dlidx < datalistlen; dlidx++)
                   {
                       ...
                       else if (cc.type == "replace")
                       {
                           ...
                           else
                           {
                               ...
                               paramChangeColumns.push(data.id);
                               paramRow = cr;
                               //trace("cc.type=replace", i, cr, callfunc, paramAllColumns, paramChangeColumns);
                           }
                       ...
                       else if (cc.type == "update")
                       {
                           ...
                           {                            
                               ...
                               paramRow = cr;
                               //trace("cc.type=update", i, cr, callfunc, paramAllColumns, paramChangeColumns);
                           }
   
                           // if (!data.hasOwnProperty(colIdx)) continue;
                   ...
                   if ((!cc.useactiveformcallback) || (cc.useactiveformcallback && (cc.objform === application.getActiveForm())))
                   {
                       ...
                       {
                           //cc.objform.$call(cc.callback, paramChangeRows.join(), null, paramobjChangeList, "DATA");
   ...
   */
                    ...
                    else if (cc.type == "update")
                    {
                        ...
                        for (colId in row)
                        {
                            ...
                            if (value != row[colId])
                            {
                                ...
                                //trace("checkFieldname : " + checkFieldname + " colId : " + colId + " value : " + value + " cc.check : " + cc.check);
    ...
    //===============================================================
    //_pXPush._onsuccess = function (classtype, reason, messagetype, messagekey, returnvalue)
    _pXPush._onsuccess = function (objData)
    {
        ...
        /*
    	    _pXPush._onsuccess = function (objData)
    {
        var e = new nexacro.XPushEventInfo("onsuccess", objData.reason, objData.serverip, objData.serverport, this.getObject(objData.Objectid));
    	...
    	*/
        ...
        var e = new nexacro.XPushEventInfo("onsuccess", objData.reason, objData.serverip, objData.serverport, command, pushmessageobject);
        this.connectip = objData.serverip;
        this.connectport = objData.serverport;
    ...
    _pXPush._onerror = function (objData)
    {
		...
		var errormsg = this._geterrormsg(this.errorcode);
        var e = new nexacro.XPushErrorEventInfo("onerror", this.errorcode, errormsg, objData.serverip, objData.serverport, command, pushmessageobject);
	...
	_pXPush._geterrormsg = function (errorcode)
    {
        ...
        {
        	codename = "object_push_fail_adui_unui";
        }
