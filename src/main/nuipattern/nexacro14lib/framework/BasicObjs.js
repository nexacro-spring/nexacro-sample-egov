...
if (!nexacro.Image)
{
    ...
    _pImageObject.set_src = function (v)
    {
        ...
        {
            this.src = v;
            this._handle = nexacro._getImageObject(v, this.on_load, this); //, this.on_loading, this); //, this.on_loading, this);
        }
    };

    //html5 : width, height 가 정상(>=0)일때는 _handle 이 node 
    //        width, height 가 비정상(<0)일때는 _handle 이 null  errstatus
    //runtime : width, height 가 정상(>=0)일때는 _handle 이 없음
    //        width, height 가 비정상(<0)일때는 _handle 이 errstatus
...
if (!nexacro.HotKey)
{
    ...
    _pHotKey._modifierkey = 0;  // CTRL,ALT,SHIFT
    _pHotKey._keycode = 0;      // A,B,C,D, ...
