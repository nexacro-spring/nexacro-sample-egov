...
if (nexacro.Browser == "Runtime")
{
    if (!nexacro._init_platform_runtime)
    {
	    ...
	    {
            // 확인필요 mousemove가 엄청나게 들어옴
            /*
	        if (_window._cur_client_pos.x == windowX &&
	    ...
	    nexacro._syshandler_onsyscommand_forward = function(_window, elem, command)
	    {
	        ...
	        if (_window._handle) nexacro.__refreshDirtyWindow(_window._handle);
	        return ret;
	    };
	    ...
	    {        
	        ...
	        if (parent_win) parent_handle = parent_win._handle;
        ...
        nexacro._createModalWindowHandle = function(parent_win, _window, name, left, top, width, height, resizable, layered, lockmode)
        {
            ...
            // lock windows
		    var window_list = [ application.mainframe._getWindow() ]; // 모든 modeless의 rootWindow list
		        for (var i = 0; i < application.popupframes.length; i++)
		        {
		            var popup_frame = application.popupframes[i];
		            var popup_win = popup_frame._window;
		        if (popup_win == this || popup_win == null) // null == showModal ?
		                continue;
        ...
        // 해당 window만 disable & modal overlay 생성
        ...
        nexacro._getHoverElement = function (_win, clientX, clientY)
        {
            ...
            //var hover_elem = nexacro.__getElementFromPoint(_win._handle, pointX, pointY);
