...
if (nexacro.Browser != "Runtime")
{
    if (!nexacro._init_platform_HTML5)
    {
	    ...
	    nexacro.isTouchInteraction = (nexacro.Browser == "MobileSafari" || nexacro.OS == "Android" || nexacro.OS == "iOS" || nexacro.OS == "Windows Phone"); //nexacro.OS == "wp7"
    ...
    if (nexacro.Browser == "IE")
    {
        nexacro._createSysEvent_ForwardFuncs = function (_cur_win)
        {
            ...
            if (nexacro.BrowserVersion <= 8) // under ie8
            {
    ...
    else if (nexacro.Browser != "IE")
    {
        nexacro._createSysEvent_ForwardFuncs = function (_cur_win)
        {
            ...
            _cur_win._syshandler_onorientationchange_forward = function (evt)
            {
                ...
                if (nexacro.OS == "Android" && reset_viewport)// && nexacro.Browser != "Chrome")
                {
    ...
    else if (nexacro.Browser != "IE")
    {
	    ...
	    nexacro._finalizeHTMLSysEvent = function (_cur_win, id)
	    {
	        ...
	        if (application._is_attach_childframe)
	        {
	            ...
	            if (htmlsysevent && htmlsysevent.length == 0) htmlsysevent = null;                
	        }
	        else
	    ...
	    nexacro.__attachChildFrame = function (_cur_win, _doc, key, adl_url, div_id, fdl_url)
	    {
	        ...
	        if (_doc.getElementById(name)) return;
	        ...
	        if (!application._is_loaded)
	        {
	            ...
	            application._RegisterClass(_global_context);
	        }
	        ...
	        application._project_url = nexacro._getProjectBaseURL(fdl_url);
	        nexacro._project_url = application._project_url;
            ...
            // adl이 먼저 올라 와야 하므로 fdl delay를 건다.
	        setTimeout(function ()
	        {
	    ...
	    nexacro._syshandler_onmousedown = function (_sysEvent, node, evt)
	    {
		    ...
		    if (application._is_attach_childframe)
		    {
		        ...
		        for (var i = 0; i < len; i++)
		        {
		            ...
		            if (sysEvent._custom_node_id == id) continue;
		    ...
		    if (win && elem)
		    {
		        ...
		        if (evt.button == (application._is_attach_childframe ? nexacro_HTMLSysEvent[id].MOUSE_LBUTTON : nexacro_HTMLSysEvent.MOUSE_LBUTTON))
			    {
				    ...
				    //nexacro._observeSysEvent(elem._handle, "losecapture", "onlosecapture", _sysEvent._syshandler_onmouseup_forward);
			        //elem._handle.setCapture();
		        ...
		        else if (evt.button == (application._is_attach_childframe ? nexacro_HTMLSysEvent[id].MOUSE_RBUTTON : nexacro_HTMLSysEvent.MOUSE_RBUTTON) || evt.button == 0) // 0 을 rbutton 으로 인식함
		        {
    ...
    else if (nexacro.Browser != "IE")
    {
	    ...
	    nexacro._syshandler_onmousewheel = function (_sysEvent, node, evt)
	    {
	        ...
	        {
	            // e.clientX, e.clientY는 win ClientArea left,top 0을 기준으로 측정된 값임
	            var ret = win._on_sys_mousewheel(elem, nexacro.__getWheelDeltaX(evt), nexacro.__getWheelDeltaY(evt), nexacro._getSysEventBtnString({ button: 1, which: 2 }), evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY);
	            if (ret === false)
	            {
	                nexacro._stopSysEvent(evt);
	            }
        ...
        nexacro._syshandler_ontouchmove = function (_sysEvent, node, evt, orgevt)
	    {
		    ...
		    if (nexacro.OS == "Android" && nexacro.Browser == "Safari")
		    {
...
// 	        var entered = application._entered;
// 	        if (!win || !entered || entered != elem)
// 	        {
// 	            application._entered = elem;
// 	        };

	        elem = (elem && elem.setSelectEventPassElement) ? elem.setSelectEventPassElement(evt.clientX, evt.clientY) : elem;
	        var from_elem = _sysEvent._cur_over_elem;

	        // Browser에 따라 같은 component에서 to_elem과 elem이 다른경우 있으므로 component까지 추가검사
	    ...
	    if (nexacro.Browser != "IE" || (nexacro.Browser == "IE" && nexacro.OSVersion >= 6.0))// NT6.0 : Vista
	    {
	            ...
	            if (w != win.clientWidth || h != win.clientHeight)
	            {
	                ...
	                if (nexacro.OS == "Android" && win._is_active_window)
	                {
	                    ...
	                    if (win._last_focused_elem &&
                            (win._last_focused_elem instanceof nexacro.InputElement || win._last_focused_elem instanceof nexacro.TextAreaElement))
    ...
    else
    {
	    ...
	    nexacro._createWindowHandle = function(parent_win, target_win, name, left, top, width, height, resizable, layered, taskbar, is_main)
	    {
	            ...
	            else
	            {
	                ...
	                if (parent_win) parent_handle = parent_win._dest_handle;
	    ...
	    nexacro.__createOpenWindowHandle = function (_handleParent, name, formurl, left, top, width, height, resizable, /*[nouse]*/layered, /*[nouse]*/taskbar)
	    {
	        if (left == null)
    ...
    else if (nexacro.Browser == "iOS" && nexacro.Browser == "MobileSafari")
    {
        ...
        nexacro._getWindowHandleClientHeight = function (_win_handle, id)
        {
            if (application._is_attach_childframe)
                return _win_handle.document.getElementById(id).clientHeight;
			//iOS & MobileSafari에서 keyboard 올라온 후 회전(Landscape)시 _win_handle.innerHeight 비정상 값(iOS bug) 
            return _win_handle.document.documentElement.clientHeight;
        };
    }
    else
    ...
    if (!nexacro.isTouchInteraction)
    {
        ...
        {
            var doc = _win._doc;
            if (application._is_attach_childframe && doc) return doc.getElementById(id).clientWidth;
        ...
        {
            var doc = _win._doc;
            if (application._is_attach_childframe && doc) return doc.getElementById(id).clientHeight;
    ...
    else
    {
        nexacro._getMainWindowWidth = function (_win, id)
        {
            ...
            if (doc)
            {
                if (application._is_attach_childframe) return doc.getElementById(id).clientWidth;
    ...
    else  //localStorage
    {
                        ...
                        if (type == "object")
	    {
                        ...
                        jsondata[key] = { "type": type, "value": varValue + "" };
                    }
    ...
    else
    {
        ...
        {
            // firefox의 경우 좌표값이 없으면 예외가 발생
            // 나머지 브라우저는 임의의 node가 리턴.
        ...
        nexacro._getEvtId = function (evt)
        {
            ...
            {
                var id = evt.id;
                if (!id) id = evt.currentTarget ? evt.currentTarget.id : 0;
            }
        ...
        nexacro._device_exception_table = [
            {
                // 기본값
                orientationchange_reset_viewport: (nexacro.OS == "Android") ? true : false, // orientationchange시 viewport를 리셋하면 안되는 경우 false로 지정
                swap_screen: (nexacro.OS == "Android") ? true : false, // orientationchange시 screen.width,height값이 서로 swap되면 true로 지정
                ...
                is_portrait_device: // android phone이면 기본적으로 세로로 길쭉한 장비라고 가정함. 그외는 undefined
                    (nexacro.OS == "Android") ? 
                        (((nexacro.Browser == "Runtime" && nexacro.__isPhone && nexacro.__isPhone()) || (nexacro.Browser != "Runtime" && nexacro._isMobile())) ?
                            (true) : (undefined)
                        )
                        : (undefined),
                reset_viewport_delay: 0,
                use_windowsize_as_bodysize: false,
                set_vp_twice_ranged_scale: true, // scale min~max 범위를 갖는 경우 viewport를 2번에 걸쳐 세팅 (초기화면 확대 방지)
            ...
            },
            {
                // 옵티머스 G3 A (4.4.2)
                model: "LG-F410S",
                browser: "Chrome", // 기본 브라우저도 Chrome
        ...
        // (Mobile) getBoundingClientRect bug in Chrome 43 version
