...
if (!nexacro.Radio)
{
    ...
    _pRadio.on_apply_style_cursor = function (cursor)
    {
        ...
        if (control_elem)
        {
            ...
            for (var i = 0; i < item_len; i++)
            {
                    radioitems[i].on_apply_style_cursor(cursor); //input cursor argument
            }
    ...
    _pRadio.on_created_contents = function ()
    {
        ...
        if (control_elem)
        {
            ...
            var datacolumn = this.datacolumn == "" ? this.codecolumn : this.datacolumn;// default codecolumn
    ...
    /*
    _pRadio._getDlgCode = function (keycode)
    {
        if (keycode && (keycode == nexacro.Event.KEY_UP || keycode == nexacro.Event.KEY_DOWN))
            var want_tab = this._getPreCalculateWantTab(keycode);
        else
        var want_tab = this._want_tab;
        this._want_tab = nexacro._enableaccessibility;
        return { want_tab: want_tab, want_return: false, want_escape: false, want_chars: false, want_arrows: want_tab };
    };
    */
    ...
    {        
        ...
        else if (this._innerdataset && this._items.length == 0)    // div에 radio폼이 url로 걸려있는경우 created가 호출되기 전에 set_value가 수행될때를 위함
        {
            if (this._innerdataset.getRowCount() == 0) v = -1;
        }
